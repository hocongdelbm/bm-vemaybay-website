...
class Viewreturnbooking extends SugarView {
    ...
    function populatereturnbooking($from_date, $to_date, $booking, $return_stt) {
        $sql_search = '';
        if(!empty($booking)) {
            $sql_search = ' AND bk.name = "'.$booking.'"';
        } else {
        ...
        // search theo tình trạng
        switch($return_stt) {
            case '4': // Đã hoàn
                $return_stt_con = 'HAVING total_sup_return = sup_return AND total_payment <= paid';
                break;
            case '2': // Hãng nợ
                $return_stt_con = 'HAVING total_sup_return > sup_return AND paid > 0';
                break;
            default: 
                $return_stt_con = '';
                break;
        ...
        if(empty($booking)) {
            ...
            $html .= '
                ...
                // <td class="text-center"><b>Phải thu hãng</b></td>
                //     <td class="text-center"><b>Phải trả khách</b></td>
        } //else $open_amt = 0;
        ...
        while($row = $this->bean->db->fetchByAssoc($res)) {
            ...
            for($r = 0; $r < count($return_inf); $r++) {
                ...
                // giá trị tổng cuối
                $total_payment += $return_dt[3];
                $total_sup_return += $return_dt[4];
                $tt_sup_return += $return_dt[5];
                $total_debt += $return_dt[3] - $return_dt[4];
            }

            // if($row['booking_name'] == '5SF8937605' && $GLOBALS['current_user']->id == '1') {
            //     echo $row['hv_status']; exit;
            // }
            ...
            foreach($return_arr AS $return_key => $return_val) {
                if($b_row == 0) {
                    ...
                    // $html .= '<td class="text-end">' . format_number($sup_debt) . '</td>';
                    // $html .= '<td class="text-end">' . format_number($return) . '</td>';
                    $html .= '</tr>';
                } else {
                    $html .= '
        ...
        $html .= '
            ...
            // <td class="text-end"><b>'.format_number($total_sup_debt). '</b></td>
            //     <td class="text-end"><b>'.format_number($total_returning). '</b></td>
    ...
    function getOpeningAmt($to_date, $return_stt) {
        switch($return_stt) {
            case '4': // Đã hoàn
                $sql_con = 'WHERE SUM(return_acc) <= 0';
                break;
            default:
                $sql_con = '';
                break;
    ...
    function getSupplierOpeningAmt($to_date, $return_stt) {
        switch ($return_stt) {
            case '4': // Đã hoàn
                $sql_con = 'WHERE SUM(debt) = SUM(sup_return)';
                break;
            default:
                $sql_con = '';
                break;
