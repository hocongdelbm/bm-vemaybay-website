...
class EC_HoanVeViewDetail extends ViewDetail
{
	...
	function populateCustomButtons()
	{
		...
		if (
			ACLController::checkAccess('EC_Payment_Voucher', 'edit', true)
		...
		if (ACLController::checkAccess('EC_Payment_Voucher', 'edit', true) && $this->bean->tinhtrang == '1') {
			// Chỉ hiện nút nếu còn tiền chưa chi
			$sql = "SELECT IFNULL(SUM(amount), 0) 
            FROM ec_payment_voucher 
            WHERE deleted = 0 AND pv_status = 3 
            AND hoanve_id = '" . $this->bean->id . "'";
			$paid = $this->bean->db->getOne($sql);
			$remaining = $this->bean->tongtienkhach - $paid;

			if ($remaining > 0) {
				$this->ss->assign('THEM_PHIEUCHI', $this->buildPhieuChiButton());
			}
		}
	}
	...
	private function buildPhieuChiButton()
	{

		// ID lấy từ EC_Payment_Voucher.js (đã hardcode sẵn cho "Tiền hoàn vé")
		$payment_type_id   = '7a7abc9b-0925-bdb7-d92b-526e3c5c0c14';
		$payment_type_name = 'Tiền hoàn vé';

		// Người nhận: liên hệ đầu tiên của booking
		$receipent_name = $this->getBookingContactName($this->bean->booking_id);

		// Nội dung: "Hoàn vé <mã booking>"
		$description = 'Hoàn vé ' . ($this->bean->booking ?? '');

		$data = [
			'module'         => 'EC_Payment_Voucher',
			'action'         => 'EditView',
			'amount'         => $this->bean->tongtienkhach,
			'receipent_name' => $receipent_name,
			'description'    => $description,
			'ec_payment_types_id_c' => $payment_type_id,
			'payment_type'          => $payment_type_name,
			'hoanve'         => $this->bean->name,
			'hoanve_id'      => $this->bean->id,
			'booking_id'     => $this->bean->booking_id   ?? '',
			'booking_name'   => $this->bean->booking ?? '',
		];

		return $this->buildFormButton($data, [
			'form_id' => 'frmPhieuChi',
			'btn_id'  => 'btnPhieuChi',
			'label'   => 'Tạo phiếu chi',
	...
	private function getBookingContactName($booking_id)
	{
		...
		$sql = "SELECT c.last_name, c.first_name
					...
					INNER JOIN ec_flight_bookings bk
						ON bk.contact_id = c.id AND bk.deleted = 0
					WHERE bk.id = '" . $this->bean->db->quote($booking_id) . "'
					AND c.deleted = 0
					LIMIT 1";


		$row = $this->bean->db->fetchByAssoc($this->bean->db->query($sql));

		return $row ? trim($row['last_name'] . ' ' . $row['first_name']) : '';
	}

	// ─── Helper: sinh HTML form + button ─────────────────────────────────────────

	private function buildFormButton(array $fields, array $btn, string $extra_html = '')
	{
		$hidden = '';
		foreach ($fields as $name => $value) {
			$hidden .= '<input type="hidden" name="' . $name . '" value="' . htmlspecialchars($value, ENT_QUOTES) . '" />' . "\n";
		}

		return '</form>
		<form action="index.php" method="post" id="' . $btn['form_id'] . '" name="' . $btn['form_id'] . '">
			' . $hidden . '
			' . $extra_html . '
			<input type="submit"
				class="' . $btn['class'] . '"
