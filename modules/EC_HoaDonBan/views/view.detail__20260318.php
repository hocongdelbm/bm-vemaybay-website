...
class EC_HoaDonBanViewDetail extends ViewDetail
{
	...
	public function populateLineItemsMisa()
	{
		$html = '<table class="table-details__booking" cellpadding="0" cellspacing="0" border="0">
					<thead>
						<tr>
							<th width="3%" class="text-center">#</th>
							<th width="10%" class="text-start">Mã hàng</th> 
							<th class="text-start">Tên hàng</th> 
							<th width="5%" class="text-center">TK công nợ</th>
							<th width="5%" class="text-center">TK doanh thu</th>
							<th width="5%" class="text-center">ĐVT</th>
							<th width="5%" class="text-center">SL</th>
							<th width="10%" class="text-center">Giá bán</th> 
							<th width="10%" class="text-center">Phí DV</th>
							<th width="10%" class="text-center">VAT</th>
							<th width="10%" class="text-center">Thành tiền</th>
							<th width="10%" class="text-center">Thu hộ</th>
						</tr>
					</thead>';
		...
		while ($row = $this->bean->db->fetchByAssoc($res)) {
			if ($this->bean->loaihoadon == 0) {
				...
				$html .= '<tr class="row-item row-loaihoadon-0">
							<td class="text-center" title="STT">' . (++$i) . '</td>
							<td class="text-start" title="Mã hàng">
								<a href="index.php?module=EC_Flight_Bookings&action=DetailView&record=' . $row['booking_id'] . '" target="_blank">' . $row['booking'] . '</a>
								' . $error_txt . $receipt . '
							</td>
							<td class="text-start" title="Tên hàng">' . $row['name'] . '</td>
							<td class="text-end" title="TK Công nợ"></td>
							<td class="text-end" title="TK Doanh thu"></td>
							<td class="text-end" title="ĐVT"></td>
							<td class="text-center" title="SL">' . format_number($row['soluong']) . '</td>
							<td class="text-end" title="Giá bán">' . format_number($row['dongia']) . '</td>
							<td class="text-end" title="Phí DV">' . format_number($row['phidv']) . '</td>
							<td class="text-end" title="VAT">' . format_number($row['tienthue']) . '</td>
							<td class="text-end" title="Thành tiền">' . format_number($row['thanhtien'] - $row['phithuho'] * $row['soluong']) . '</td>
							<td class="text-end text-muted" title="Thu hộ">' . format_number($row['phithuho']) . '</td>
						</tr>';
			} else if ($this->bean->loaihoadon == 1) {
				$html .= '<tr class="row-item row-loaihoadon-1">
		...
		if ($this->bean->loaihoadon == 0 && $tongthuho > 0) {
			...
			if ($phisanbay > 0) {
				$extra_qty++;
				$html .= '<tr class="row-item row-phisanbay">
							<td class="text-center" title="STT">' . (++$i) . '</td>
							<td class="text-start" title="Mã hàng"><b>Phí sân bay</b></td>
							<td class="text-start" title="Tên hàng"></td>
							<td class="text-start" title="TK Công nợ"></td>
							<td class="text-start" title="TK Doanh thu"></td>
							<td class="text-start" title="ĐVT"></td>
							<td class="text-center" title="SL">1</td>
							<td class="text-end" title="Giá bán">' . format_number($phisanbay) . '</td>
							<td class="text-end" title="Phí DV">X</td>
							<td class="text-end" title="VAT">X</td>
							<td class="text-end" title="Thành tiền">' . format_number($phisanbay) . '</td>
							<td class="text-end" title="Thu hộ">X</td>
						</tr>';
			...
			if ($phikhac > 0) {
				$extra_qty++;
				$html .= '<tr class="row-item row-phikhac">
							<td class="text-center">' . (++$i) . '</td>
			...
			$extra_qty++;
			$html .= '<tr class="row-item row-thuho">
						<td class="text-center">' . (++$i) . '</td>
						<td class="text-start"><b>Phí khác</b></td>
						<td class="text-start"></td>
						<td class="text-end">1</td>
	...
	public function populateLineItemsWin()
	{
		...
		if ($this->bean->company_unit == 'MHV') {
			...
			if ($this->bean->tinhtrang == '0' || $this->bean->tinhtrang == '1') {
				...
				foreach ($array_item_invoice['items'] as $index => $item) {
					...
					$input .= '
						<input type="hidden" name="misa_itemCode[]" value="' . $item['itemCode'] . '" />
						<input type="hidden" name="misa_itemName[]" value="' . $item['itemName'] . '" />
						<input type="hidden" name="misa_itemUnit[]" value="' . $item['itemUnit'] . '" />
						<input type="hidden" name="misa_itemQuantity[]" value="' . $item['itemQuantity'] . '" />
						<input type="hidden" name="misa_itemPrice[]" value="' . $item['itemPrice'] . '" />
						<input type="hidden" name="misa_itemVatRate[]" value="' . $item['itemVatRate'] . '" />
						<input type="hidden" name="misa_itemVatAmnt[]" value="' . $item['itemVatAmnt'] . '" />
						<input type="hidden" name="misa_itemAmountNoVat[]" value="' . $item['itemAmountNoVat'] . '" />
					';
				}
				...
				$create_invoice_btn_win = '
					<button type="button" name="btnCreateInvoice" id="btnCreateInvoice" class="btn btn-warning" onclick="showDialog(\'dialog-create-invoice\')">' . $text_button . ' Win</button>
				...
				<button type="button" class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#staticModalCreateMisaInv">
					Tạo chứng từ Misa
				</button>
				<div class="modal fade" id="staticModalCreateMisaInv" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticModalCreateMisaInvLabel" style="display: none;" aria-hidden="true">
