...
class EC_HoaDonBanViewDetail extends ViewDetail {
	...
	public function populateLineItems() {
		...
		if($this->bean->is_signed == 1) 
			$custom_sohoadon .= '<span style="color:#FF0000; float:right;">
				<svg width="18px" height="18px" stroke-width="1.75" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" color="#FF0000" style="padding-bottom:2px;">
					<path d="M7 12.5L10 15.5L17 8.5" stroke="#FF0000" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"></path><path d="M12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22Z" stroke="#FF0000" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"></path>
				</svg>
				Đã ký số
			</span>';
		...
		if($this->bean->loaihoadon == 0) {
			$html .= '<thead>
				<tr>
					<th width="3%" class="text-center"><span>STT</span></th>
					<th width="12%" class="text-center"><span>Booking</span></th>
					<th width="15%" class="text-center"><span>Số vé</span></th>
					<th width="10%" class="text-center"><span>PNR</span></th>
					<th width="7%" class="text-center"><span>Số lượng</span></th>
					<th width="14%" class="text-center"><span>Đơn giá</span></th>
					<th width="10%" class="text-center"><span>VAT</span></th>
					<th width="20%" class="text-center"><span>Thành tiền</span></th>
					<th width="6%" class="text-center text-muted"><span>Thu hộ</span></th>
				</tr>
			</thead>';
			$colspan = 4;
		} 
		...
		$i = $tonggiaban = $tongthue = $tongthuho = $phikhac = 0;
		$array_item = [
			'items' 			=> [],
		...
		while($row = $this->bean->db->fetchByAssoc($res)) {
			if($this->bean->loaihoadon == 0) {
				...
				$html .= '<tr>
					...
					<td class="text-start">' . $row['ticket_code'] . '</td>
			...
			$tongthue 	+= $row['tienthue'] * $row['soluong'];
			$tongthuho 	+= $row['phithuho'] * $row['soluong'];

			if($row['phithuho'] > 0) {
				$arr_thuho = $this->separateCollectionFees($row['phithuho'], $row['booking_id'], $row['name']);
				$phikhac += $arr_thuho['other_fee'];
			}
			...
			$array_item['items'][] = [
				...
				'itemAmountNoVat' 	=> $row['thanhtien'] - ($row['tienthue'] + $row['phithuho']),
			];
		}
		...
		if($this->bean->loaihoadon == 0 && $tongthuho > 0) {
			...
			if($phisanbay > 0 ) {
				...
				// $array_item['itemName'] 		.= '<input type="hidden" name="itemName[]" value="Phí sân bay" />';
				// $array_item['itemQuantity'] 	.= '<input type="hidden" name="itemQuantity[]" value="1" />';
				// $array_item['itemPrice'] 		.= '<input type="hidden" name="itemPrice[]" value="'.$phisanbay.'" />';
				// $array_item['itemVatRate'] 		.= '<input type="hidden" name="itemVatRate[]" value="-1" />';
				// $array_item['itemVatAmnt'] 		.= '<input type="hidden" name="itemVatAmnt[]" value="" />';
				// $array_item['itemAmountNoVat'] 	.= '<input type="hidden" name="itemAmountNoVat[]" value="'.$phisanbay.'" />';
			}
			if($phikhac > 0 ) {
				$extra_qty++;
				$html .= '<tr>
				...
				// $array_item['itemName'] 		.= '<input type="hidden" name="itemName[]" value="Phí khác" />';
				// $array_item['itemQuantity'] 	.= '<input type="hidden" name="itemQuantity[]" value="1" />';
				// $array_item['itemPrice'] 		.= '<input type="hidden" name="itemPrice[]" value="'.$phikhac.'" />';
				// $array_item['itemVatRate'] 		.= '<input type="hidden" name="itemVatRate[]" value="-1" />';
				// $array_item['itemVatAmnt'] 		.= '<input type="hidden" name="itemVatAmnt[]" value="" />';
				// $array_item['itemAmountNoVat'] 	.= '<input type="hidden" name="itemAmountNoVat[]" value="'.$phikhac.'" />';
			}
		}
		elseif($this->bean->loaihoadon == 1 && $tongthuho > 0) {
			...
			// $array_item['itemName'] 		.= '<input type="hidden" name="itemName[]" value="Phí khác" />';
			// $array_item['itemQuantity'] 	.= '<input type="hidden" name="itemQuantity[]" value="1" />';
			// $array_item['itemPrice'] 		.= '<input type="hidden" name="itemPrice[]" value="'.$tongthuho.'" />';
			// $array_item['itemVatRate'] 		.= '<input type="hidden" name="itemVatRate[]" value="-1" />';
			// $array_item['itemVatAmnt'] 		.= '<input type="hidden" name="itemVatAmnt[]" value="" />';
			// $array_item['itemAmountNoVat'] 	.= '<input type="hidden" name="itemAmountNoVat[]" value="'.$tongthuho.'" />';
		}
	...
	public function populateCustomButtons($array_item_invoice) {
		...
		if($this->bean->status != '-1') {
			...
			<form class="frmCancelInvoice" action="index.php" method="post" name="frmCancelInvoice" id="frmCancelInvoice">
				...
				<input type="hidden" name="is_cancel_invoive" value="'.$this->bean->is_cancel_invoice.'" />
		...
		if($this->bean->tinhtrang == '0' || $this->bean->tinhtrang == '1') {
			...
			foreach($array_item_invoice['items'] as $index => $item) {
				$tr .= '<tr>
					...
					<td class="text-end">'.($item['itemVatRate'] == '-1' ? 'X' : ($item['itemVatRate'] . '%')).'</td>
					...
					<form method="dialog" name="form-create-invoice">
						<div class="card">
							...
							<div class="card-body">
								...
								<p>Hình thức thanh toán <i>(Payment method)</i>: <b>TM/CK</b></p>
		...
		if($this->bean->tinhtrang == '1') {
			$Inv = new WinInvoice();
			$view_invoice_button = '
				<a class="btn btn-secondary" href="'.$Inv->get_link(['invRef' => $this->bean->name]).'" target="_blank">Xem hóa đơn</a>
			';
	...
	/** Tách phí thu hộ thành từng loại phí
	 * @author DucPham
