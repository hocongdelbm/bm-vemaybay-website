...
class EC_SMS_LogsViewDetail extends ViewDetail {
    function display() {
        $this->css();
    ...
    public function populate_custom_buttons() {
        // Nút lên lịch SMS
        if(($this->bean->type == 'send_sms_list_static' || $this->bean->type == 'send_sms_list_dynamic') && $this->bean->status == 'new' && strtotime($this->bean->send_date) > strtotime(date('d-m-Y H:i',  strtotime('+7 hours')))) {
            $button_schedule = '<button type="button" class="btn btn-warning" onclick="showDialog(\'dialog-confirm-schedule\')">Lên lịch</button>
                <dialog id="dialog-confirm-schedule" class="dialog-confirm-schedule">
                    ...
                    <form method="dialog" name="form-confirm-schedule">
                        <p class="text-confirm">Lên lịch gửi tin nhắn vào lúc <i>'.date("H:i d/m/Y", strtotime($this->bean->send_date)).'</i></p>
        ...
        // Nút lên lịch Zalo
        else if($this->bean->type == 'send_zalo_broadcast' && $this->bean->status == 'new') {
        ...
        $button_voucher = '';
        if($this->bean->type == 'send_zalo_promotion'){
    ...
    public function populate_custom_fields() {
        // Status
        if($this->bean->status == 'scheduled') $custom_status = '<i class="text-warning">Đã lên lịch</i>';
        else if($this->bean->status == 'done') $custom_status = '<i class="text-success">Đã gửi</i>';
        else if($this->bean->status == 'fail') $custom_status = '<i class="text-danger">Thất bại</i>';
        else $custom_status = '<i>Mới tạo</i>';
