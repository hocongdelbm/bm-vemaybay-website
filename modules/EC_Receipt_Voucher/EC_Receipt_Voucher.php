...
class EC_Receipt_Voucher extends Basic
{
	...
	function save($check_notify = FALSE)
	{
		...
		if (
			isset($this->booking_id) && !empty($this->booking_id)
		...
		if ($this->rv_status == '1') {
            if (!isWorkingProcessExisting($this->module_dir, $this->id, 'paid')) {
		...
		if ($this->rv_status == '1' && isset($this->delivery_man_id) && !empty($this->delivery_man_id)) {
            if (!isWorkingProcessExisting($this->module_dir, $this->id, 'ticket_delivery')) {
		...
		if ($is_tele == 1) {
			...
			try {
				...
				$message .= "- Phiếu thu: **$this->name**";
				$message .= "- Loại thu: **". $app_list_strings['loai_thu_list'][(int)$this->loai_thu] ."**";
