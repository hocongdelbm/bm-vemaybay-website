		...
		<td>
           {$title}
            <table cellspacing="2">
                ...
                <tr>
                    <td colspan="3">
                        <input type="button" class="btn btn-save" value="{$MOD.LBL_BTN_SAVE}" id="renameSaveBttn" onclick='validateForm();'name="{$MOD.LBL_BTN_SAVE}" />
                        <input type="button" class="btn btn-danger" value="{$MOD.LBL_BTN_CANCEL}"  id="renameCancelBttn" name="{$MOD.LBL_BTN_CANCEL}" onclick="document.editdropdown.action.value='index'; document.editdropdown.module.value='Administration';document.editdropdown.submit()" />
                    </td>
                </tr>
            </table>

            <div class="box-section mt-3">
<table width="100%" border="0" cellspacing="0" cellpadding="0" class='edit view table-config' >
<tr>
    <td>
...
<div name='tabDropdown' id='tabDropdown' class="d-flex flex-wrap align-items-center mt-2">
{counter start=0 name="rowCounter" print=false assign="rowCounter"}
{foreach from=$dropdown item="value" key="key"}
    <span id='slot{$rowCounter}b' class="d-flex gap-2 align-items-center w-25 my-2 cursor-pointer">
