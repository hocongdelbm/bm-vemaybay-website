...
class ViewBackups extends SugarView
{
    ...
    public function display()
    {
        ...
        if ($run == "done") {
            ...
            echo '<div class="decs-section">'.$mod_strings['LBL_BACKUP_INSTRUCTIONS_1'].' <br> '.$mod_strings['LBL_BACKUP_INSTRUCTIONS_2'].' </div>'; ?>
            <form action="<?php print($form_action); ?>" method="post">
            <div class="box-section">
            <table class="table-config table-backup">
            <tr>
        ...
        <?php
            switch ($run) {
                case "confirm":
        ?>
                    <input class="btn btn-primary" type="submit" value="<?php echo $mod_strings['LBL_BACKUP_CONFIRM']; ?>" />
        <?php
                    break;
                case "confirmed":
        ?>
