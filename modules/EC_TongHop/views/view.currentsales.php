    ...
    function populateContent($smartyobj)
    {
        ...
        if (is_admin($current_user) || $current_user->title == 'QuanLy' || in_array($current_user->id, $array_iduser)) {
            ...
            $html .= '<thead>
                        <tr>
                            <th>Số vé</th>
                            <th class="hide-mobile">% Số vé</th>
                            <th>Doanh số</th>
                            <th class="hide-mobile">Chưa xuất</th>
                            <th>Vé quốc tế</th>
                            <th>Booking</th>
                            <th class="hide-mobile">% Booking</th>
                            <th class="hide-mobile">% Hủy Booking</th>
                        </tr>
                        </thead>
                    <tr>
                        <td class="text-center"><img src="themes/SuiteP/images/modules/ec_flight_booking/ticket.svg" alt="ticket" border="0" width="80" /></td>
                        <td class="text-center hide-mobile"><img src="themes/SuiteP/images/modules/ec_flight_booking/ticket_percent.png" alt="ticket_percent" border="0" width="80" /></td>
                        <td class="text-center"><img src="themes/SuiteP/images/modules/ec_flight_booking/money-bag.svg" alt="note" border="0" width="80" /></td>
                        <td class="text-center hide-mobile"><img src="themes/SuiteP/images/modules/ec_flight_booking/warning.svg" alt="warning" border="0" width="80" /></td>
                        ...
                        <td class="text-center hide-mobile"><img src="themes/SuiteP/images/modules/ec_flight_booking/success_booking_ticket.svg" alt="success_booking_ticket" border="0" width="80" /></td>
                        <td class="text-center hide-mobile"><img src="themes/SuiteP/images/modules/ec_flight_booking/cancel_booking_ticket.svg" alt="cancel_booking_ticket" border="0" width="80" /></td>
                    </tr>
                ';
            $html .= '<tr class="footer-tr">
                            <td><span class="detail_domestic">' . format_number($row['soluongve']) . '</span> / ' . $total_quantity . '</td>
                            <td class="hide-mobile">' . (empty($total_quantity) ? 0 : format_number(($row['soluongve']) * 100 / $total_quantity)) . '%</td>
                            <td>' . format_number($ds_noidia) . '</td>
                            <td class="hide-mobile">' . format_number($paid_booking['total_tkt']) . '</td>
                            <td><span class="detail_inter">' . format_number($sove_inter) . '</span> / ' . format_number($ds_inter) . '</td>
                            <td>' . format_number($total_bookings) . '</td>
                            <td class="hide-mobile">' . (empty($total_bookings) ? 0 : format_number($total_bookings_succes * 100 / $total_bookings)) . '%</td>
                            <td class="hide-mobile">' . (empty($total_bookings) ? 0 : format_number($total_bookings_cancel * 100 / $total_bookings)) . '%</td>
                        </tr>
                        <tr class="footer-tr">
                            <td class="text-start bg-yellow">Tổng doanh số trong ngày</td>
        ...
        while ($row = $db->fetchByAssoc($res)) {
            if ($row['booking_status'] == '3' || !empty($row['total_not_issued']) && $row['total_not_issued'] < $row['total_tkt']) {
                ...
                $html .= '<tr>
    				...
    				<td align="left">' . $row['contact_name'] . '</td>
    				<td align="left">' . $row['phone'] . '</td>
    				<td align="left" class="hide-mobile">' . $row['description'] . '</td>
    				<td align="center">' . format_number($row['total_tkt']) . '</td>
