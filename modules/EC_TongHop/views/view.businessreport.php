...
class Viewbusinessreport extends SugarView
{
    ...
    function genInforReport($period) {
        ...
        foreach ($date_ranges as $range) {
            ...
            ';
            if ((int)$count_rows == 0 && strtotime($from_date) <= strtotime(date('Y-m-d')) ) {
                // CREATE
                ...
                while ($row = $db->fetchByAssoc($results)) {
                    ...
                    if ((int)$user_exists > 0) {
                        $update_sql = '
                            UPDATE ec_report_weekly
                            SET 
                                bk_created = ' . $row['bk_created'] . ',
                                bk_called = ' . $row['bk_called'] . ',
                                bk_paying = ' . $row['bk_paying'] . ',
                                bk_confirmed = ' . $row['bk_confirmed'] . ',
                                bk_printed = ' . $row['bk_printed'] . ',
                                bk_completed = ' . $row['bk_completed'] . ',
                                bk_cancelled = ' . $row['bk_cancelled'] . ',
                                total_qty = ' . $row['total'] . ',
            ...
            $html .= '<table class="table-details__booking table-report__weekly" cellpadding="0" cellspacing="0">
                        <thead>';
                            if (in_array($period, ['this_month', 'previous_month'])) {
                                ...
                                $html .= '
                                    <tr>
                                        <th colspan="8">Từ ' . $start_day . ' (' . $start_dayInVietnamese . ') đến ' . $end_day . ' (' . $end_dayInVietnamese . ')</th>
                                    </tr>';
                            } else {
                                $html .= '
                                    <tr>
                                        <th colspan="8">' . date('d-m-Y', strtotime($from_date)) . ' (' . $this->getDayInVietnamese($from_date) . ')</th>
                                    </tr>';
                            }
            ...
            $sql_get = "SELECT * FROM ec_report_weekly WHERE from_date = '$from_date' AND to_date = '$to_date'";
            ...
            if($row_count > 0) {
                while ($row = $this->bean->db->fetchByAssoc($res)) {
                    ...
                    $btn_add_qc = '<button class="btn btn-primary btn-add-qc-cost" data-bs-toggle="modal" data-bs-target="#modalEditCostQc" data-from-date="' . $from_date . '" data-to-date="' . $to_date . '" data-user-id="' . $row['user_id'] . '" data-site="' . $row['last_name'] . '" data-time="' . $data_time . '">
