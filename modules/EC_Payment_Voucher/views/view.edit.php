...
class EC_Payment_VoucherViewEdit extends ViewEdit
{
	...
	function customFields()
	{
		...
		// ── Nhận dữ liệu tự động từ phiếu hoàn ──────────────────────────────
		if (!empty($_POST['amount']) && empty($this->bean->id)) {
			$this->bean->amount = (float) str_replace(',', '', $_POST['amount']);
		}
		if (!empty($_POST['receipent_name']) && empty($this->bean->receipent_name)) {
			$this->bean->receipent_name = $_POST['receipent_name'];
		}
		if (!empty($_POST['description']) && empty($this->bean->description)) {
			$this->bean->description = $_POST['description'];
		}
 
		// Loại chi (relate field): cần set cả ID lẫn tên hiển thị
		if (!empty($_POST['ec_payment_types_id_c']) && empty($this->bean->ec_payment_types_id_c)) {
			$this->bean->ec_payment_types_id_c = $_POST['ec_payment_types_id_c'];
			// Tên hiển thị trên relate field
			$this->bean->payment_type = $_POST['payment_type'] ?? '';
		}
 
		// Hoàn vé
		if (!empty($_POST['hoanve_id']) && empty($this->bean->hoanve_id)) {
			$this->bean->hoanve_id   = $_POST['hoanve_id'];
			$this->bean->hoanve = $_POST['hoanve'] ?? '';
		}
 
		// Booking
		if (!empty($_POST['booking_id']) && empty($this->bean->booking_id)) {
			$this->bean->booking_id   = $_POST['booking_id'];
			$this->bean->booking = $_POST['booking_name'] ?? '';
		}
		// ─────────────────────────────────────────────────────────────────────
