...
class UsersViewEdit extends ViewEdit
{
    ...
    public function display()
    {
        ...
        $this->ss->assign('USER_ADMIN',is_admin_for_module($current_user,'Users')&& !is_admin($current_user));
        // gán giá trị password email vào biến PASSWORD_EMAIL
        $this->ss->assign('PASSWORD_EMAIL', $this->bean->password_email);
        ...
        if (!empty($this->bean->id)) {
            ...
            if (!empty($this->bean->reports_to_id)) {
                ...
                $reportsToUserField = "<input class='box-input' type='text' name='reports_to_name' id='reports_to_name' value='{$reportsToUser}' disabled>\n";
...
EOD
        ...
        $action_button_header[] = <<<EOD
                    <input	title="{$APP['LBL_CANCEL_BUTTON_TITLE']}" id="CANCEL_HEADER" accessKey="{$APP['LBL_CANCEL_BUTTON_KEY']}"
                              class="btn btn-danger" onclick="var _form = $('#EditView')[0]; _form.action.value='{$RETURN_ACTION}'; _form.module.value='{$RETURN_MODULE}'; _form.record.value='{$RETURN_ID}'; _form.submit()"
