...
class UserViewHelper
{
    ...
    protected function setupButtonsAndTabs()
    {
        ...
        if (!$this->bean->is_group) {
            ...
            $buttons_header[] = "<input type='button' class='btn btn-warning' id='reset_user_preferences_header' onclick='if(confirm(\"{$reset_pref_warning}\"))window.location=\"" . $_SERVER['PHP_SELF'] . '?' . $user_preference_url . "&reset_preferences=true\";' value='" . translate('LBL_RESET_PREFERENCES', 'Users') . "' />";
            $buttons_header[] = "<input type='button' class='btn btn-warning' id='reset_homepage_header' onclick='if(confirm(\"{$reset_home_warning}\"))window.location=\"" . $_SERVER['PHP_SELF'] . '?' . $the_query_string . "&reset_homepage=true\";' value='" . translate('LBL_RESET_HOMEPAGE', 'Users') . "' />";

            $buttons_footer[] = "<input type='button' class='btn btn-warning' id='reset_user_preferences_footer' onclick='if(confirm(\"{$reset_pref_warning}\"))window.location=\"" . $_SERVER['PHP_SELF'] . '?' . $user_preference_url . "&reset_preferences=true\";' value='" . translate('LBL_RESET_PREFERENCES', 'Users') . "' />";
            $buttons_footer[] = "<input type='button' class='btn btn-warning' id='reset_homepage_footer' onclick='if(confirm(\"{$reset_home_warning}\"))window.location=\"" . $_SERVER['PHP_SELF'] . '?' . $the_query_string . "&reset_homepage=true\";' value='" . translate('LBL_RESET_HOMEPAGE', 'Users') . "' />";
        }
    ...
    protected function setupAdvancedTabNavSettings()
    {
        ...
        foreach ($chooser->args['values_array'][0] as $key => $value) {
            $chooser->args['values_array'][0][$key] = isset($app_list_strings['moduleList'][$key]) ? $app_list_strings['moduleList'][$key] : '';
        }

        foreach ($chooser->args['values_array'][1] as $key => $value) {
            $chooser->args['values_array'][1][$key] = isset($app_list_strings['moduleList'][$key]) ? $app_list_strings['moduleList'][$key] : '';
        }
