...
tabElem.find('a em').css('color','');});if(invalidTabs.length>0){$('#tab'+invalidTabs[0]).parent().click();$('#tab'+invalidTabs[0]+' em').css('color','red');}};function verify_data(form){var tabsState=saveTabsState();var extraMessages=[];var isError=false;try{isError=!check_form("EditView");}catch(e){console.warn('check_form("EditView") failed, continuing with field-level checks only:',e);}
if(trim(form.last_name.value)==""){add_error_style('EditView',form.last_name.name,SUGAR.language.get('app_strings','ERR_MISSING_REQUIRED_FIELDS')+SUGAR.language.get('Users','LBL_LIST_NAME'));isError=true;}
...
if($('#mandatory_email').length&&trim($('#mandatory_email').text())!==''){var hasEmail=false;var $emailInputs=$('#EditView .email-address-line-container:not(.template) input[type=email]');$emailInputs.each(function(){if(trim($(this).val())!==''){hasEmail=true;}});if(!hasEmail){var emailMissingMsg=SUGAR.language.get('app_strings','ERR_MISSING_REQUIRED_FIELDS')+SUGAR.language.get('app_strings','LBL_EMAIL_ADDRESS_BOOK_EMAIL_ADDR');var $firstEmailInput=$emailInputs.first();if($firstEmailInput.length){add_error_style('EditView',$firstEmailInput.get(0),emailMissingMsg);}else{extraMessages.push(emailMissingMsg);}
isError=true;}}
if(isError==true){var messages=extraMessages.slice();$('#EditView .validation-message').each(function(){var text=trim($(this).text());if(text!==''&&$.inArray(text,messages)===-1){messages.push(text);}});alert(messages.length>0?messages.join('\n'):SUGAR.language.get('app_strings','ERR_MISSING_REQUIRED_FIELDS'));restoreTabsState(tabsState);return false;}
...
function bindPasswordVisibilityToggles(){$('.pwd-toggle').off('click.pwdToggle').on('click.pwdToggle',function(){var $toggle=$(this);var $input=$('#'+$toggle.data('target'));if($input.length===0){return;}
var reveal=$input.attr('type')==='password';$input.attr('type',reveal?'text':'password');$toggle.toggleClass('is-revealed',reveal).attr('aria-pressed',reveal?'true':'false').attr('aria-label',reveal?SUGAR.language.get('Users','LBL_HIDE_PASSWORD'):SUGAR.language.get('Users','LBL_SHOW_PASSWORD'));});}