...
function testOutboundSettings() {
  ...
  if (
    document.getElementById("mail_smtpuser") &&
    trim(document.getElementById("mail_smtpuser").value) == ""
  ) {
    isError = true;
    errorMessage +=
      SUGAR.language.get("app_strings", "LBL_EMAIL_ACCOUNTS_SMTPUSER") +
      "<br/>";
  }
...
function sendTestEmail() {
  ...
  if (trim(fromAddress) == "") {
    ...
    overlay(
      SUGAR.language.get("app_strings", "ERR_INVALID_REQUIRED_FIELDS"),
      SUGAR.language.get(
        "app_strings",
        "LBL_EMAIL_SETTINGS_FROM_TO_EMAIL_ADDR",
      ),
  ...
  overlay(
    SUGAR.language.get("app_strings", "LBL_EMAIL_PERFORMING_TASK"),
  ...
  var callbackOutboundTest = {
    success: function (o) {
      hideOverlay();
      overlay(
        SUGAR.language.get("app_strings", "LBL_EMAIL_TEST_OUTBOUND_SETTINGS"),
        SUGAR.language.get("app_strings", "LBL_EMAIL_TEST_NOTIFICATION_SENT"),
        "alert",
      );
    },
  };
  ...
  if (
    document.getElementById("mail_smtpuser") &&
    document.getElementById("mail_smtppass")
  ) {
    var postDataString =
      "mail_sendtype=SMTP&mail_smtpserver=" +
      smtpServer +
      "&mail_smtpport=" +
      mail_smtpport +
      "&mail_smtpssl=" +
      mail_smtpssl +
      "&mail_smtpauth_req=true&mail_smtpuser=" +
      trim(document.getElementById("mail_smtpuser").value) +
      "&mail_smtppass=" +
      trim(document.getElementById("mail_smtppass").value) +
      "&outboundtest_from_address=" +
      fromAddress +
      "&outboundtest_to_address=" +
      toAddress;
  } else {
    var postDataString =
      "mail_sendtype=SMTP&mail_smtpserver=" +
  ...
  YAHOO.util.Connect.asyncRequest(
    "POST",
    "index.php?action=testOutboundEmail&mail_name=system&module=EmailMan&to_pdf=true&sugar_body_only=true",
...
function testOutboundSettingsDialog() {
  ...
  if (!EmailMan.testOutboundDialog) {
    EmailMan.testOutboundDialog = new YAHOO.widget.Dialog(
      "testOutboundDialog",
      {
        modal: true,
        visible: true,
...
function verify_data(form) {
  ...
  try {
    ...
    console.warn(
      'check_form("EditView") failed, continuing with field-level checks only:',
      e,
    );
  }
  ...
  if (trim(form.last_name.value) == "") {
    add_error_style(
      ...
      form.last_name.name,
      SUGAR.language.get("app_strings", "ERR_MISSING_REQUIRED_FIELDS") +
        SUGAR.language.get("Users", "LBL_LIST_NAME"),
    );
    isError = true;
  }
  if (trim(form.user_name.value) == "") {
    add_error_style(
      ...
      form.user_name.name,
      SUGAR.language.get("app_strings", "ERR_MISSING_REQUIRED_FIELDS") +
        SUGAR.language.get("Users", "LBL_USER_NAME"),
    );
    isError = true;
  }

  if (
    document.getElementById("required_password").value == "1" &&
    document.getElementById("new_password").value == ""
  ) {
    add_error_style(
      "EditView",
      form.new_password.name,
      SUGAR.language.get("app_strings", "ERR_MISSING_REQUIRED_FIELDS") +
        SUGAR.language.get("Users", "LBL_NEW_PASSWORD"),
    );
    isError = true;
  }

  if (
    $("#mandatory_email").length &&
    trim($("#mandatory_email").text()) !== ""
  ) {
    var hasEmail = false;
    var $emailInputs = $(
      "#EditView .email-address-line-container:not(.template) input[type=email]",
    );
  ...
  if (
    document.EditView.return_id.value != "" &&
    typeof form.reports_to_id != "undefined" &&
    document.EditView.return_id.value == form.reports_to_id.value
  ) {
  ...
  if (
    document.EditView.dec_sep.value != "" &&
    document.EditView.dec_sep.value == "'"
  ) {
    alert(
      SUGAR.language.get("app_strings", "ERR_NO_SINGLE_QUOTE") +
        SUGAR.language.get("Users", "LBL_DECIMAL_SEP"),
    );
    restoreTabsState(tabsState);
    return false;
  }
  ...
  if (
    document.EditView.num_grp_sep.value != "" &&
    document.EditView.num_grp_sep.value == "'"
  ) {
    alert(
      SUGAR.language.get("app_strings", "ERR_NO_SINGLE_QUOTE") +
        SUGAR.language.get("Users", "LBL_NUMBER_GROUPING_SEP"),
    );
    restoreTabsState(tabsState);
    return false;
  }
  ...
  if (document.EditView.num_grp_sep.value == document.EditView.dec_sep.value) {
    alert(
      SUGAR.language.get("app_strings", "ERR_DECIMAL_SEP_EQ_THOUSANDS_SEP"),
    );
    restoreTabsState(tabsState);
    return false;
  }
  if (
    document.getElementById("portal_only") &&
    document.getElementById("portal_only") == "1" &&
    typeof document.getElementById("new_password") != "undefined" &&
    typeof document.getElementById("new_password").value != "undefined"
  ) {
    if (
      document.getElementById("new_password").value != "" ||
...
function onUserEditView() {
  YAHOO.util.Event.onContentReady("user_theme_picker", function () {
    document.getElementById("user_theme_picker").onchange = function () {
      document.getElementById("themePreview").src =
        ...
        document.getElementById("user_theme_picker").value +
        "&imageName=themePreview.png";
      if (
        typeof themeGroupList[
          document.getElementById("user_theme_picker").value
        ] != "undefined" &&
  ...
  setSymbolValue(
    document.getElementById("currency_select").options[
      document.getElementById("currency_select").selectedIndex
    ].value,
  );
  setSigDigits();
...
function bindPasswordVisibilityToggles() {
  $(".pwd-toggle")
    ...
    .on("click.pwdToggle", function () {
      ...
      $toggle
        .toggleClass("is-revealed", reveal)
        .attr("aria-pressed", reveal ? "true" : "false")
        .attr(
          "aria-label",
          reveal
            ? SUGAR.language.get("Users", "LBL_HIDE_PASSWORD")
            : SUGAR.language.get("Users", "LBL_SHOW_PASSWORD"),
        );
    });
}
