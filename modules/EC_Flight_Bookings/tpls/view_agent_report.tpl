    ...
    <script>
        $(document).ready(function () {
            ...
            $('#checkall').change(function () {
                ...
                if (is_check_all) {
                    $('input:checkbox[name="agent_id[]"]').attr('checked', true);
                } else {
                    $('input:checkbox[name="agent_id[]"]').attr('checked', false);
                }
            });
            ...
            $('#btnViewDetail').on('click', function () {
                ...
                if ($('input:checkbox[name="agent_id[]"]:checked').length == 0) {
                    alert('Bạn chưa chọn đối tượng để tiếp tục');
                    return false;
                }
            });
...
<form action="index.php" method="post">
    ...
    <input type="hidden" name="action" value="agentreport">
    <input type="hidden" name="print" value="true"/>
    <input type="hidden" name="from_date" value="{$POST_FROM_DATE}"/>
    <input type="hidden" name="to_date" value="{$POST_TO_DATE}"/>
    <input type="hidden" name="agtcode_{$AGENT_ID}" value="{$AGENT_CODE}"/>
    <input type="hidden" name="agtname_{$AGENT_ID}" value="{$AGENT_NAME}"/>
...
<h1 class="title">CÔNG NỢ PHẢI THU</h1>

<form action="index.php" class="box-section" method="post" name="frmSearch" id="frmSearch" target="_blank">
    <input type="hidden" name="module" value="EC_Flight_Bookings"/>
    <input type="hidden" name="action" value="agentreport"/>
    <input type="hidden" name="print" value="true"/>
    ...
    <div class="action--wrap flex-wrap d-flex gap-2 align-items-center">
        ...
        <div class="from-to-date--wrap d-inline-flex gap-2 align-items-center">
            ...
            <div class="d-flex gap-2 align-items-center date_trigger--wrap tdate_trigger--wrap">
                ...
                <div class="dateTime d-flex gap-2 position-relative">
                <input  class="date_input box-input" type="text" maxlength="10" size="11" title="" value="{$POST_TO_DATE}" id="to_date" name="to_date" autocomplete="off">
        ...
        <div class="button-action--wrap mb-0">
            <input type="submit" id="btnSearch" name="btnSearch" class="btn btn-primary" value="Tìm" title="Tìm"/>
        </div>
    </div>

    <table class="table-data table-details__booking mt-3" cellpadding="0" cellspacing="0" border="0">
        <thead>
            <tr>
                <th width="5%"><input type="checkbox" id="checkall" value="0"/></th>
                <th width="15%">Mã đối tượng</th>
                <th width="30%">Tên đối tượng</th>
                <th width="30%" class="hide-mobile">Địa chỉ</th>
                <th width="10%" class="hide-mobile">Điện thoại</th>
