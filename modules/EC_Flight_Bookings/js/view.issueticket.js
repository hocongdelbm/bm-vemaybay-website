...
$(document).ready(function () {
  ...
  $("#btnSearch").click(function () {
    ...
    $.ajax({
      ...
      success: function (response) {
        try {
          ...
          if (objRes.status == 1) {
            ...
            showModalNotify(
              "error",
              objRes.message ?? "Lỗi trong quá trình lấy dữ liệu",
            );
            clearBooking();
      ...
      error: function (xhr, status, error) {
        ...
        if (status === "timeout")
          errorMessage =
            "Thời gian phản hồi từ máy chủ quá lâu. Vui lòng thử lại.";
        ...
        success: function (response) {
          ...
          try {
            if (response.status == 1) {
              ...
              showModalNotify(
                "warning",
                response.message ?? "Lỗi trong quá trình lấy dữ liệu",
              );
  ...
  $("#payNowButton").on("click", function () {
    ...
    if (
      confirm(
        `Tiến hành thanh toán ${bookingCode} hãng ${systemCode}\nTổng tiền: ${unpaidAmount}`,
      )
    ) {
      ...
      $.ajax({
        ...
        error: function (xhr, status, error) {
          ...
          showModalNotify(
            "error",
            "Xử lý thanh toán không thành công. Vui lòng thử lại",
          );
          resetPaymentButton();
        },
      });
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (response.status) {
            ...
            showModalNotify(
              "error",
              response.message ?? "Hủy đặt chỗ không thành công",
            );
          }
        },
        error: function (xhr, status, error) {
          ...
          showModalNotify(
            "error",
            "Hủy đặt chỗ không thành công. Vui lòng thử lại",
          );
        },
      });
    }
  ...
  $(".status-badge").on("click", function () {
    ...
    modalEl
      .querySelector(`#${dataAction}Confirm`)
      .addEventListener("click", (event) => {
        const entryClass = $('input[name="entryClass"]').val();
        const bookingCode = $('input[name="bookingCode"]').val();
        const systemCode = $('input[name="systemCode"]').val();
        const listTicket = Array.from(
          modalEl.querySelectorAll(
            `input[name="checkbox-${dataAction}-ticket"]`,
          ),
        ).map((cb) => cb.value);
        const listTicketChecked = Array.from(
          modalEl.querySelectorAll(
            `input[name="checkbox-${dataAction}-ticket"]:checked`,
          ),
        ).map((cb) => cb.value);
        ...
        if (listTicket.length == listTicketChecked.length)
          confirmMessage = `${dataTooltip} với tất cả số vé`;
          ...
          $.ajax({
            ...
            success: function (response) {
              ...
              if (response.status) {
                ...
                showModalNotify(
                  "error",
                  response.message ?? `${dataTooltip} không thành công`,
                );
              }
            },
            error: function (xhr, status, error) {
              ...
              showModalNotify(
                "error",
                `${dataTooltip} không thành công. Vui lòng thử lại`,
              );
            },
          });
        }
...
function renderBookingInfo(data) {
  ...
  // Status
  const statusDetails = isBookingExpired(
    data.BookingStatus,
    data.BookingExpired,
  )
    ? getBookingStatusDetails("timeout")
    : getBookingStatusDetails(data.BookingStatus);
  $("#bookingStatus")
...
function updateStatusIcons(data) {
  ...
  if ("IsPaid" in data) {
    if (data.IsPaid) {
      ...
      paidBadge.attr(
        "data-tooltip",
        "Tất cả các khoản thanh toán đã được xử lý thành công",
      );
    } else {
...
function getBookingStatusDetails(bookingStatus) {
  const statusMap = {
    ...
    "update-required": {
      ...
      description:
        "Cần cập nhật thông tin booking sau khi xuất vé và cần thanh toán",
    },
    "change-paid": {
      class: "change-paid",
  ...
  return bookingStatus in statusMap
    ? statusMap[bookingStatus]
    : {
        class: "unknown",
        text: `Không xác định`,
  ...
  listPassenger.forEach(function (passenger) {
    ...
    if (
      ...
      for (let i = 0; i < Object.keys(data.ListFlight).length; i++) {
        if ((i == 0 && purchasedBaggageDep) || (i == 1 && purchasedBaggageRet))
          continue;
 ...
 * @returns
 */
function createBaggageServiceDialog(
  baggageData,
  passengerData,
  systemCode,
  direction,
  ...
  let passengerName =
    `${passengerData.LastName} ${passengerData.FirstName}`.trim();
  ...
  return new Promise((resolve) => {
            ...
            <div class="baggage-dialog">
                <div class="baggage-header">
                    <h2 class="d-flex align-center">
                        ...
                        <img class="ms-3" src="${getLinkImageAirline(systemCode)}" alt="${systemCode}" style="height:40px;" />
                    </h2>
    ...
    if (
      (!("ListBaggage" in baggageData) && !baggageData.ListBaggage) ||
      baggageData.ListBaggage.length == 0
    ) {
      baggageListEl.innerHTML =
        "<center><i>Không có hành lý để thêm</i></center>";
    ...
    confirmBtn.addEventListener("click", () => {
      const selectedRadio = wrapper.querySelector(
        'input[name="baggageOption"]:checked',
      );
          ...
          success: function (response) {
            try {
              ...
              if (response.status == 1) {
                ...
                showModalNotify(
                  "error",
                  response.message ?? "Lỗi thêm hành lý, vui lòng thử lại",
                );
                console.error(response);
              }
            } catch (e) {
              ...
              showModalNotify(
                "error",
                "Lỗi thêm hành lý, vui lòng thử lại",
                e.message,
              );
              console.error(e);
            }
