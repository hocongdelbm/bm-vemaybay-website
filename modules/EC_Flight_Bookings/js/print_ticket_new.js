$(document).ready(function () {
  ...
  $(document).on("click", ".btnPrintEticketNew", function () {
    ...
    if (hasPerPaxChanges) {
      ...
      try {
        var perPaxData = JSON.parse(
          $popup.attr("data-per-pax-itineraries") || "[]",
        );
          ...
          for (var t = 0; t < pax.itineraries.length; t++) {
            ...
            $itiContainer.append(
              '<label style="display:flex; align-items:center; gap:8px; padding:5px 0; cursor:pointer; border-bottom:1px solid #f0f0f0; font-size:13px;">' +
                '<input type="checkbox" class="popup-perpax-iti-check" value="' +
                iti.id +
                '" data-direction="' +
                iti.direction +
                '" data-pax-id="' +
      ...
      $("#tbl_pax tr.psg-line:not(.luggage)").each(function () {
        ...
        if ($luggageRow.length > 0) {
          ...
          if ($luggageRow.find("td").text().trim().length > 50)
            luggageText += "...";
        }
      ...
      for (var k = 0; k < psgOrder.length; k++) {
        ...
        $passengerList.append(
          '<label style="display:flex; align-items:center; gap:8px; padding:6px 0; cursor:pointer; border-bottom:1px solid #f0f0f0;">' +
            '<input type="checkbox" class="popup-check-psg" value="' +
            psg.passId +
            '" data-type="' +
            psg.passType +
            '" checked style="width:16px;height:16px;cursor:pointer;" />' +
        ...
        if (itinerariesData) {
          ...
          for (var i = 0; i < itineraries.length; i++) {
            ...
            $itineraryList.append(
              '<label style="display:flex; align-items:center; gap:8px; padding:6px 0; cursor:pointer; border-bottom:1px solid #f0f0f0;">' +
                '<input type="checkbox" class="popup-check-iti" value="' +
                iti.id +
                '" data-direction="' +
                iti.direction +
                '" data-ticket-type="' +
  ...
  // Submit → open form / url in new tab
  $(document).on("click", "#btnSubmitPrintNew, #btnSubmitSendNew", function () {
    var $popup = $("#dlgPrintTicketNew");
    var hasPerPaxChanges = $popup.attr("data-has-per-pax-changes") === "1";
    var passengers = [];
    var itineraries = [];
    ...
    var khuhoi =
      directions.indexOf("0") !== -1 && directions.indexOf("1") !== -1 ? 1 : 0;
    ...
    var isSend = $(this).attr("id") === "btnSubmitSendNew";

    if (isSend) {
      // Build absolute form to sendeticketnew
      var form = $("<form>", {
        method: "POST",
        action: "index.php",
        target: "_blank",
