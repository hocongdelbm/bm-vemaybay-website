$(document).ready(function () {
  ...
  const INVOICE_INF_FIELDS = [
    { name: "iv_account_name", type: "text" },
    { name: "company_name", type: "textarea" },
    { name: "tax_code", type: "text" },
    { name: "iv_email", type: "text" },
    { name: "iv_identity_number", type: "text", maxlength: 12 },
    { name: "company_address", type: "textarea" },
    {
  ...
  $(document).on("click", "#btnEditInvoiceInf", function (e) {
    e.preventDefault();
    e.stopPropagation();
    startEditInvoiceInf($(this));
  });

  function startEditInvoiceInf($icon) {
    $icon.hide();

    $.ajax({
      url: "index.php?entryPoint=entryPointFlightBookings",
      type: "POST",
      data: { booking_id: bookingId, for: "getInvoiceInfJson" },
      beforeSend: function () {
      ...
      success: function (response) {
        ...
        if (!response || !response.success) {
          showModalNotify(0, "Không tải được thông tin hoá đơn");
          $icon.show();
          return;
        }
  ...
  function exitInvoiceInfEditMode() {
    if (!invoiceInfEditState) return;
    invoiceInfEditState.fields.forEach(function (f) {
      f.$container.html(f.originalHtml);
    });
  ...
  function enterInvoiceInfEditMode(data, banks, $icon) {
    const fields = INVOICE_INF_FIELDS.map(function (field) {
      ...
      if (field.type === "textarea") {
        $input = $('<textarea class="box-textarea" rows="3"></textarea>').val(
          value,
        );
      ...
      $input.addClass("invoice-inf-input");
    ...
    const $actionsRow = $(
      '<div class="col-12 detail-view-row-item invoice-inf-actions-row">' +
        '<button type="button" class="btn btn-secondary btn-sm" id="btnCancelInvoiceInf">Hủy</button> ' +
  ...
  $(document).on("click", "#btnCancelInvoiceInf", function (e) {
    e.preventDefault();
    e.stopPropagation();
    exitInvoiceInfEditMode();
  });

  $(document).on("click", "#btnSaveInvoiceInf", function (e) {
    e.preventDefault();
    e.stopPropagation();
    if (!invoiceInfEditState) return;

    const payload = { booking_id: bookingId, for: "saveInvoiceInfInline" };
    invoiceInfEditState.fields.forEach(function (f) {
      payload[f.name] = f.$container.find(".invoice-inf-input").val();
    });

    if (
      payload.iv_email &&
      !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(payload.iv_email)
    ) {
      showModalNotify(2, "Email không đúng định dạng");
      return;
    }
    ...
    $.ajax({
      ...
      success: function (response) {
        if (!response || !response.success) {
          showModalNotify(
            0,
            (response && response.message) || "Lưu không thành công",
          );
          return;
        }
      ...
      if ($(".line_pass").length > 0) {
        ...
        passengers.forEach((element) => {
          if ($(".line_pass" + element).length == 0) {
            getPassengerLine(
              $("form[name='DetailView']>input[name='record']").val(),
              element.toString(),
              "insert",
            );
          }
  ...
  $(document).on("click", "#update_revenue", function () {
    if (!confirm("Bạn có chắc chắn muốn cập nhật doanh số cho booking này?"))
      return false;
    else {
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response == 1) {
            ...
            let text_warning =
              "Cập nhật thất bại. Vui lòng liên hệ IT để được hỗ trợ.";
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response == 1) {
          ...
          showModalNotify(
            2,
            "Vui lòng cập nhật thông tin cuộc gọi trước khi liên kết",
          );
        } else if (response == 400) {
          showModalNotify(
            2,
            "SĐT hoặc ID của Booking không xác định. Kiểm tra lại thông tin SĐT hoặc liên hệ IT để được hỗ trợ!",
          );
        } else {
          let text_warning =
            "LK thất bại. Không tìm thấy cuộc gọi phù hợp để liên kết.";
          showModalNotify(0, text_warning);
          $(".modal-overlay, .btn-modal-close").addClass("reload");
        }
      },
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response == 1) {
          ...
          let text_warning =
            "Lỗi khi thực hiện nhấn nút remind. Vui lòng liên hệ IT để được hỗ trợ.";
          showModalNotify(0, text_warning);
          $(".modal-overlay, .btn-modal-close").addClass("reload");
        }
      },
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response == 1) {
            ...
            $select.val(prevVal);
            let text_warning =
              "Lỗi khi thực hiện thay đổi trạng thái checkin. Vui lòng liên hệ IT để được hỗ trợ.";
            showModalNotify(0, text_warning);
            $(".modal-overlay, .btn-modal-close").addClass("reload");
          }
        },
  ...
  $("#frmTicketExported, #frmCompleted, #frmChangeStatus").submit(function (e) {
    var booking_status = $(this)
      .find('input:hidden[name="booking_status"]')
      .val();
    if ($(this).find('select[name="booking_status"]').length > 0) {
      booking_status = $(this)
        .find('select[name="booking_status"] :selected')
        .val();
    }

    if (booking_status == "7" || booking_status == "8") {
      var is_ticket_exported = $(this)
        .find('input:hidden[name="is_ticket_exported"]')
        .val(); //1
      ...
      var bought_price = document.getElementsByName(
        "check_total_bought_price[]",
      ); // giá mua
      ...
      for (var j = 0; j < bought_price.length; j++) {
        ...
        if (
          $.trim(bought_price[j].value) == "" ||
          unformatNumber(bought_price[j].value) < 0
        ) {
          price_err++;
        }
  ...
  tag_textarea.keyup(function (event) {
    if (event.keyCode == 13 && event.shiftKey) {
      ...
      let height =
        parseInt($(MESSAGE_LIST_CHAT_ID).height()) - HEIGHT_ROW_TEXTAREA;
      if (rows <= 5) {
        $(this).attr("rows", rows);
        $(MESSAGE_LIST_CHAT_ID).height(height);
      }
    } else {
      // Xuống hàng do độ dài
      ...
      if (1 < count_row && count_row <= 5 && count_row > rows) {
        ...
        let height =
          parseInt($(MESSAGE_LIST_CHAT_ID).height()) - t * HEIGHT_ROW_TEXTAREA;
        $(this).attr("rows", count_row);
        $(MESSAGE_LIST_CHAT_ID).height(height);
      }
    }
  });
  tag_textarea.bind("paste", function (e) {
    // Copy paste
    ...
    if (key == 8 || key == 46) {
      ...
      if (count_row - 1 > 0) {
        ...
        if (count_row < 5 && count_row < rows) {
          ...
          let height =
            parseInt($(MESSAGE_LIST_CHAT_ID).height()) +
            t * HEIGHT_ROW_TEXTAREA;
          $(this).attr("rows", count_row);
          $(MESSAGE_LIST_CHAT_ID).height(height);
        }
      } else {
      ...
      if ($(this).val().length == 0)
        $(MESSAGE_LIST_CHAT_ID).height(HEIGHT_MESSAGE_LIST_CHAT);
    }
  });
  ...
  $("#icon-send-notes").click(function () {
    ...
    let send_loading =
      '<div class="lds-ring-notes"><div></div><div></div><div></div><div></div></div>';
  ...
  $("#confirm_delete_message").click(function (event) {
    ...
    $(".action-remove[data-id-note=" + id_note + "]")
      .closest(".row-mess")
  ...
  $("#btnCancelled").click(function () {
    ...
    $("#txtGhiChuThangThua").text(
      $('input:radio[name="radWinLoseReason"]:checked').attr("txt"),
    );
  ...
  $(".win-lose-radio").on(
    "change",
    'input:radio[name="radWinLoseReason"]',
    function () {
      $("#txtGhiChuThangThua").text(
        $('input:radio[name="radWinLoseReason"]:checked').attr("txt"),
      );
      if ($("#txtWorkingProcessNote").length != 0)
        $("#txtWorkingProcessNote").text(
          $('input:radio[name="radWinLoseReason"]:checked').attr("txt"),
        );
    },
  );
  ...
  $("#btnDongY").click(function () {
    ...
    $("#" + which_form + ' input:hidden[name="lydothangthua_id"]').val(
      $('input:radio[name="radWinLoseReason"]:checked').val(),
    );
    $("#" + which_form + ' input:hidden[name="ghichuthangthua"]').val(
      $.trim($("#txtGhiChuThangThua").val()),
    );
    $("#" + which_form).submit();
  });
  ...
  $(".frmBookingStatus").submit(function (e) {
    ...
    var booking_status = $(
      `#${frmSaveWorkingProcess} input:hidden[name="booking_status"]`,
    ).val();

    if ($(`#${frmSaveWorkingProcess} input:hidden[name="bonus"]`).length > 0) {
      $("#txtBonus").val(
        $(`#${frmSaveWorkingProcess} input:hidden[name="bonus"]`).val(),
      );
      $("#txtBonus").parent().parent().show();
    }

    if (booking_status == "8" && frmSaveWorkingProcess == "frmCompleted") {
      let complete_ok = parseInt(
        $(`#${frmSaveWorkingProcess} input:hidden[name="complete_ok"]`).val(),
      );
      if (!complete_ok) {
        showModalNotify(
          2,
          "Vui lòng điền đầy đủ giá bán, giá mua hành lý để hoàn tất",
        );
        return;
      }
      $(this).submit();
  ...
  $(document).one("click", "#btnSaveWorkingProcess", function (event) {
    ...
    if (
      ...
      $(`#${frmSaveWorkingProcess} input:hidden[name="lydothangthua_id"]`).val(
        $('input:radio[name="radWinLoseReason"]:checked').val(),
      );
      $(`#${frmSaveWorkingProcess} input:hidden[name="ghichuthangthua"]`).val(
        $.trim($("#txtWorkingProcessNote").val()),
      );
    }
    ...
    if (des.length < 20) {
      ...
      let text_warning =
        'Bạn note quá ít! Trang sẽ tự động reload trong vòng <span id="count-down" class="fw-semibold color-red"> ' +
        countdownAndReload(10) +
        "</span> nữa.";
      showModalNotify(0, text_warning);
      $(".modal-overlay, .btn-modal-close").addClass("reload");
      ...
      $("#dlgWorkingProcessNote").dialog("close");
      let text_warning =
        'Bạn note quá nhiều! Trang sẽ tự động reload trong vòng <span id="count-down" class="fw-semibold color-red"> ' +
        countdownAndReload(10) +
        "</span> nữa.";
      showModalNotify(0, text_warning);
      $(".modal-overlay, .btn-modal-close").addClass("reload");
      ...
      save_post_data +=
        "&txtWorkingProcessNote=" + $.trim($("#txtWorkingProcessNote").val());
      ...
      $.ajax({
        ...
        success: function (res) {
          ...
          if (res == 1) {
            ...
            $("#save-working-process-error").text(
              "Lỗi xảy ra trong quá trình lưu. Vui lòng thử lại",
            );
            $(".container-waiting").hide();
  ...
  $("#add_luggage_btn").click(function () {
    $.ajax({
      ...
      data:
        ...
        $(".ticket_class1").text() +
        "&contact_name=" +
        $("#btnCheckContactInfo").attr("ct_name") +
        "&contact_phone=" +
        $("#btnCheckContactInfo").attr("ct_mobile"),
      beforeSend: function () {
        $("body").css({ cursor: "wait" });
      },
      success: function (response) {
  ...
  $("#change_flight_time").click(function () {
    $.ajax({
      ...
      type: "POST",
      data:
        "for=changeFlightTime&id=" +
        $("form[name='DetailView']>input[name='record']").val(),
      beforeSend: function () {
        $("body").css({ cursor: "wait" });
        $("#line_itineraries_area").html(
          "<center><i>Vui lòng chờ trong giây lát...</i></center>",
        );
      },
      success: function (response) {
    ...
    $("#tbl_change_flight_time").dialog({
      ...getResponsiveDialogOptions(1023),
      title:
        "Đối ngày bay / hành trình / hành khách / hành lý / số vé / code vé",
    });
  });

  $(document).on(
    "change",
    'select[name="pass_luggage_ob[]"], select[name="pass_luggage_ib[]"]',
    function () {
      ...
      if (name == "pass_luggage_ob[]") {
        $('input[name="pass_luggage_price[]"]')
          .eq(index)
          .val(formatNumber(dataCost));
      } else if (name == "pass_luggage_ib[]") {
        $('input[name="pass_luggage_price_inbound[]"]')
          .eq(index)
          .val(formatNumber(dataCost));
      }
    },
  ...
  $(document).on("click", "#btnRenderQRCode", function () {
    ...
    const amount =
      parseInt($("#new_payment_amount").val().replace(/\./g, "")) || 0;
  ...
  $("#btn-mapping-call-booking").click(function () {
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (response == 1) {
            ...
            if (
              confirm(
                "Cuộc gọi này đã liên kết với BK, bạn có chắc chắn liên kết không?",
              )
            ) {
              doMapCall(1);
            }
            ...
            showModalNotify(
              0,
              "Thao tác không thành công. Không tìm thấy cuộc gọi để liên kết",
            );
          }
        },
      });
    ...
    if (alreadyLinked === "1") {
      let confirmMsg =
        "Cuộc gọi này đã liên kết với BK" +
        (linkedBooking.length > 0 ? " " + linkedBooking : "") +
        ", bạn có chắc chắn liên kết không?";
  ...
  $("#manualLinkCall").on("shown.bs.modal", function () {
    ...
    $("input[name=call_name]")
      .val("")
      .removeData("already-linked")
      .removeData("linked-booking");
    ...
    $("#suggested-calls-list").html(
      '<p class="text-muted small">Đang tìm kiếm...</p>',
    );
    ...
    $.ajax({
      ...
      success: function (calls) {
        ...
        $.each(calls, function (i, call) {
          let alreadyLinked =
            call.booking_id && call.booking_id.length > 0 ? "1" : "0";
          let linkedBookingName = call.booking_name_linked
            ? call.booking_name_linked
            : "";
          let linkedBadge =
            alreadyLinked === "1"
              ? '<span class="badge bg-warning text-dark ms-2">Đã liên kết: ' +
                $("<div>").text(linkedBookingName).html() +
                "</span>"
              : "";
          html +=
            '<button type="button" class="list-group-item list-group-item-action py-2 mb-2 w-100 suggest-call-item"' +
            ' data-call-name="' +
            $("<div>").text(call.name).html() +
            '"' +
            ' data-already-linked="' +
            alreadyLinked +
            '"' +
            ...
            $("<div>").text(call.name).html() +
            "</span>" +
            '<span class="form-label small ms-2">' +
            (call.date_start || "") +
            "</span>" +
            linkedBadge +
            "</button>";
  ...
  $(document).on("click", ".suggest-call-item", function () {
    ...
    $("input[name=call_name]")
      .val(callName)
      .data("already-linked", $(this).attr("data-already-linked"))
      .data("linked-booking", $(this).attr("data-linked-booking"));
  ...
  $(".btn-use-voucher").on("click", function () {
    let id = $(this).attr("for");
    let bookingId =
      $(this).attr("booking_id") ||
      $("#btn_apply_booking_voucher").attr("booking_id") ||
      $(`#${id}`).attr("booking_id");

    if (!bookingId) {
      showBookingVoucherToast(
        "error",
        "Không lấy được thông tin booking. Vui lòng tải lại trang và thử lại.",
      );
      return;
    }
    ...
    setBookingVoucherMessage(
      '<p class="booking-voucher-empty">Đang tải danh sách mã giảm giá phù hợp...</p>',
    );
  ...
  function normalizeBookingVoucherItem(voucher) {
    ...
    return {
      ...
      discount_type:
        ...
        (parseInt(voucher.reduce_percent || 0, 10) > 0 ? "percent" : "amount"),
  ...
  function formatBookingVoucherConditionLines(condition) {
    ...
    if (condition.for_phone_value) {
      lines.push(
        `Áp dụng cho SĐT ${escapeBookingVoucherHtml(condition.for_phone_value)}`,
      );
    }
    if (parseInt(condition.min_order_value || 0, 10) > 0) {
      lines.push(
        `Đơn tối thiểu ${formatNumber(condition.min_order_value)} VND`,
      );
    }
    if (parseInt(condition.number_of_tickets || 0, 10) > 0) {
      lines.push(
        `Booking từ ${escapeBookingVoucherHtml(condition.number_of_tickets)} vé trở lên`,
      );
    }
    if (condition.flight_type) {
      lines.push(
        `Chỉ áp dụng cho chuyến ${condition.flight_type === "domestic" ? "Nội địa" : "Quốc tế"}`,
      );
    }
    if (parseInt(condition.ticket_type || 0, 10) > 0) {
      lines.push(
        `Chỉ áp dụng cho vé ${parseInt(condition.ticket_type, 10) === 2 ? "Khứ hồi" : "Một chiều"}`,
      );
    }
    if (condition.journey && String(condition.journey).length > 6) {
      lines.push(
        `Hành trình áp dụng: ${escapeBookingVoucherHtml(String(condition.journey).replace(/,/g, ", "))}`,
      );
    }
  ...
  function renderBookingVoucherCondition(voucherKey) {
    ...
    let title = escapeBookingVoucherHtml(
      voucher.campaign_name || voucher.code || "",
    );
    let code = escapeBookingVoucherHtml(voucher.code || "");
    let expiryDate = escapeBookingVoucherHtml(
      formatBookingVoucherExpiry(voucher.end_time || ""),
    );
    let discountLabel = escapeBookingVoucherHtml(
      getBookingVoucherDiscountLabel(voucher),
    );
    let conditionLines = formatBookingVoucherConditionLines(
      voucher.condition_voucher,
    );
    let conditionHtml =
  ...
  function upsertBookingVoucherOption(voucher, selected) {
    ...
    let isAlreadyApplied =
      ...
      !!bookingVoucherState.applied[item.key];
    if (isAlreadyApplied) {
    ...
    if (selected && item.selectable) {
      ...
      bookingVoucherState.selected[item.key] =
        bookingVoucherState.options[item.key];
    }
  }
  ...
  function resetBookingVoucherState(bookingId) {
    ...
    let appliedVouchers = decodeBookingVoucherPayload(
      $(BOOKING_VOUCHER_SELECTORS.dialog).attr("data-applied-vouchers"),
    );
  ...
  function getBookingVoucherItems() {
    return Object.keys(bookingVoucherState.options)
      .map(function (voucherKey) {
        return bookingVoucherState.options[voucherKey];
      })
      .sort(function (firstVoucher, secondVoucher) {
        let firstApplied = bookingVoucherState.applied[firstVoucher.key]
          ? 1
          : 0;
        let secondApplied = bookingVoucherState.applied[secondVoucher.key]
          ? 1
          : 0;
        ...
        let firstDiscount =
          parseInt(firstVoucher.discount_amount || 0, 10) || 0;
        let secondDiscount =
          parseInt(secondVoucher.discount_amount || 0, 10) || 0;
        if (firstDiscount !== secondDiscount) {
          return secondDiscount - firstDiscount;
        }

        return String(firstVoucher.code || "").localeCompare(
          String(secondVoucher.code || ""),
        );
      });
  }

  function updateBookingVoucherFooter() {
    let selectedItems = Object.keys(bookingVoucherState.selected).map(
      function (voucherKey) {
    ...
    $(BOOKING_VOUCHER_SELECTORS.selectedAmount).text(
  ...
  function buildAppliedBookingVoucherTags(vouchers) {
    ...
    vouchers.forEach(function (voucher) {
      ...
      let invalidReason = escapeBookingVoucherHtml(
        voucher.applied_invalid_reason || "",
      );
  ...
  function updateBookingVoucherStateAfterSave(data) {
    ...
    $(BOOKING_VOUCHER_SELECTORS.dialog).attr(
      "data-applied-vouchers",
      encodeBookingVoucherPayload(appliedVouchers),
    );
  ...
  function renderBookingVoucherItem(voucher) {
    ...
    let disabledReason = escapeBookingVoucherHtml(
      voucher.disabled_reason || "Không áp dụng cho booking này",
    );
    let bestBadge = renderBookingVoucherBadge(
      voucher.is_best_in_group,
      "booking-voucher-best-badge",
      "Tốt nhất",
    );
    let appliedBadge = renderBookingVoucherBadge(
      voucher.applied,
      "booking-voucher-applied-badge",
      "Đang áp dụng",
    );
    let invalidReason = escapeBookingVoucherHtml(
      voucher.applied_invalid_reason || "",
    );
    let invalidBadge = renderBookingVoucherBadge(
      invalidReason,
      "booking-voucher-invalid-badge",
      invalidReason,
    );
  ...
  function renderBookingVoucherList() {
    ...
    if (vouchers.length === 0) {
      setBookingVoucherMessage(
        '<p class="booking-voucher-empty">Không có mã giảm giá phù hợp.</p>',
      );
      updateBookingVoucherFooter();
      return;
    }
  ...
  function renderBookingVoucherOptions(obj) {
    if (obj.error != 0) {
      setBookingVoucherMessage(
        `<p class="booking-voucher-empty text-danger">${escapeBookingVoucherHtml(obj.message || "Không tải được danh sách mã giảm giá")}</p>`,
      );
      return;
    }
  ...
  function loadBookingVoucherOptions(bookingId) {
    $.ajax({
      ...
      error: function () {
        setBookingVoucherMessage(
          '<p class="text-danger">Không tải được danh sách mã giảm giá. Bạn có thể nhập mã để kiểm tra thủ công.</p>',
        );
      },
    });
  }
  ...
  $(document).on(
    "click",
    ".booking-voucher-item, .booking-voucher-check",
    function (event) {
      event.preventDefault();
      event.stopPropagation();
      ...
      let voucherKey =
        $(this).attr("data-voucher-key") ||
        $(this).closest(".booking-voucher-item").attr("data-voucher-key");
      ...
      if (bookingVoucherState.options[voucherKey].selectable === false) {
        showBookingVoucherToast(
          "warning",
          bookingVoucherState.options[voucherKey].disabled_reason ||
            "Mã giảm giá không áp dụng cho booking này",
        );
        return;
      }
  ...
  $("#btn_check_booking_voucher").click(function () {
    ...
    $.ajax({
      ...
      success: function (obj) {
        ...
        showBookingVoucherToast(
          "error",
          obj.message || "Mã giảm giá không hợp lệ",
        );
      },
      error: function (XMLHttpRequest, textStatus, errorThrown) {
        $(".container-waiting").hide();
  ...
  $("#btn_apply_booking_voucher").click(function () {
    ...
    Object.keys(bookingVoucherState.selected).forEach(function (voucherKey) {
      if (bookingVoucherState.selected[voucherKey].voucher_id) {
        selectedVoucherIds.push(
          bookingVoucherState.selected[voucherKey].voucher_id,
        );
      }
    });
    ...
    if (
      Object.keys(bookingVoucherState.selected).length > 0 &&
      selectedVoucherIds.length === 0
    ) {
      showBookingVoucherToast(
        "error",
        "Không lấy được ID mã giảm giá. Vui lòng tải lại trang và thử lại.",
      );
      return false;
    }
      ...
      success: function (obj) {
        ...
        if (obj.error != 0) {
          showBookingVoucherToast(
            "error",
            obj.message ||
              "Thao tác không thành công. Liên hệ IT để được hỗ trợ.",
          );
          return;
        }
        ...
        showBookingVoucherToast(
          "success",
          obj.message || "Cập nhật mã giảm giá thành công",
        );
      },
      error: function (XMLHttpRequest, textStatus, errorThrown) {
        $(".container-waiting").hide();
      ...
      $.ajax({
        ...
        success: function (res) {
          if ("status" in res && res.status === 1) {
            $('input[type="checkbox"][name="customer_source"]').prop(
              "checked",
              false,
            );
            $(`input#customer_source_${customer_source}`).prop("checked", true);
          } else {
            $(`input#customer_source_${customer_source}`).prop(
              "checked",
              false,
            );
          }
  ...
  if (regexlatlong.test(latlong)) {
    ...
    $.ajax({
      ...
      beforeSend: function () {
        $("#city").append(
          `<i id="location-loading" class="ms-2" style="color:#a7a7a7;">Đang định vị...</i>`,
        );
      },
      success: function (res) {
  ...
  function extractZaloId(rawValue) {
    ...
    const uidMatch =
      value.match(/[?&]uid=(\d+)/i) || value.match(/\buid=(\d+)/i);
...
function checkNoteLine() {
  ...
  for (var i = 0; i < arr.length; i++) {
    if (
      $("#note_deleted" + i).val() == "0" &&
      $.trim($("#note_desc" + i).val()) == ""
    ) {
  ...
  if (type == 0) {
    ...
    if (bkd_arr.length > 0) {
      for (var i = 0; i < bkd_arr.length; i++) {
        if (
          bkd_arr[i].value == "0" &&
          unformatNumber($("#bkd_quantity" + i).val()) <= 0
        ) {
        ...
        if (
          bkd_arr[i].value == "0" &&
          unformatNumber($("#bkd_total_price" + i).val()) <= 0
        ) {
        ...
        if (bkd_arr[i].value == "0") {
          // lượt đi
          if (
            $("#bkd_supplier_id" + i).val() != "" &&
            $("#bkd_direction" + i).val() == "0"
          ) {
            bkd_has_supplier_outb[$("#bkd_passenger_type" + i).val()][1] +=
              parseInt($("#bkd_quantity" + i).val());
          }
          ...
          if ($("#flight_type").val() == "0") {
            if (
              $("#bkd_supplier_id" + i).val() != "" &&
              $("#bkd_direction" + i).val() == "1"
            ) {
              bkd_has_supplier_inb[$("#bkd_passenger_type" + i).val()][1] +=
                parseInt($("#bkd_quantity" + i).val());
            }
          }
        }
    ...
    for (i = 0; i < name_arr.length; i++) {
      if (
        $.trim($("#new_psg_full_name" + i).val()) == "" ||
        $.trim($("#new_psg_full_name" + i).val().length) < 5
      ) {
    ...
    // check thông tin đổi ngày bay, hành trình
    ...
    if (iti_hour_arr.length > 0) {
      ...
      addToValidate(
        "tbl_change_flight_time",
        "departure_date0",
        "date",
        false,
        "Ngày phải nhập theo cú pháp: 28-02-2022",
      );
      addToValidate(
        "tbl_change_flight_time",
        "arrival_date0",
      ...
      if (!check_form("tbl_change_flight_time")) {
        ...
        var date1 =
          departure_date_arr[1] +
          "-" +
          departure_date_arr[0] +
          "-" +
          departure_date_arr[2];
        var date2 =
          arrival_date_arr[1] +
          "-" +
          arrival_date_arr[0] +
...
function createNewLineDetail(type) {
  if (type < 3) {
    ...
    for (i = 0; i < detail_arr.length; i++) {
      if (type == 1) {
        if (
          $("#psg_luggage_price" + i).val() != "0" ||
          $("#psg_luggage_price_inbound" + i).val() != "0"
        ) {
      ...
      if (type == 2) {
        if (
          $("#new_psg_full_name" + i).val() != $("#psg_full_name" + i).val()
        ) {
          $("#psg_detail_id" + i).val("");
          $("#psg_add_type" + i).val(type);
        } else {
          $("#new_psg_full_name" + i).val(
            $("#psg_full_name" + i).attr("old_name"),
          );
          $("#psg_parent_detail_id" + i).val("");
        }
      }
...
function getItiLine(booking_id, iti_id = "") {
  $.ajax({
    ...
    beforeSend: function () {
      $("#line_itineraries_area").html(
        "<div class='line_pass'>Please wait for a moment...</div>",
      );
    },
    success: function (response) {
      $("#line_itineraries_area").html(response);
    ...
    success: function (response) {
      ...
      $("input[name='pass_id[]']").each(function (index) {
        ...
        addToValidate(
          ...
          "date",
          false,
          "Ngày phải nhập theo cú pháp: 01-01-2022",
        );
      });
...
function calculateLineEditDetails(ln, is_cal_admin = 0, is_cal_tax = 0) {
  ...
  if (ticket_type != "2") {
    ...
    if (
      ((airline_inf[0] == "VNA" ||
        airline_inf[0] == "VNP" ||
        airline_inf[0] == "BBA") &&
        $("#bk_edit_direction" + ln).val() == 0) || // lượt đi
      ((airline_inf[1] == "VNA" ||
        airline_inf[1] == "VNP" ||
        airline_inf[1] == "BBA") &&
        $("#bk_edit_direction" + ln).val() == 1) // lượt về
    ) {
      if (is_cal_tax) {
    ...
    if (
      (airline_inf[0] != "VNA" &&
        airline_inf[0] != "VNP" &&
        $("#bk_edit_direction" + ln).val() == 0) ||
      (airline_inf[1] != "VNA" &&
        airline_inf[1] != "VNP" &&
        $("#bk_edit_direction" + ln).val() == 1 &&
        is_cal_admin)
    ) {
  ...
  let supplier_discount = unformatNumber(
    $("#bk_edit_supplier_discount" + ln).val(),
  );
  let supplier_ticketing_fee = unformatNumber(
    $("#bk_edit_supplier_ticketing_fee" + ln).val(),
  );
  let total_price = 0;
...
function calculateTotal() {
  ...
  var total_bought_price = document.getElementsByName(
    "bkd_total_bought_price[]",
  );
  ...
  var discount_amount = unformatNumber(
    $.trim($("#discount_amount span.discount_value").text()),
  );
