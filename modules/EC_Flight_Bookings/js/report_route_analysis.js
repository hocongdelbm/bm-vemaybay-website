 ...
 * Dữ liệu động được nạp qua window.REPORT_ROUTE (in inline trong template).
 */
(function ($) {
    $(function () {
        ...
        $(document).on('click', '.main-line td.period-col', function (e) {
            ...
            detailRow.find('.route-item').each(function () {
                ...
                linkOk.text(bkOk);
                linkAll.text(bkAll);
                $r.find('.show-bk-list').attr('data-period', periodLabel).attr('data-fromdate', fromF).attr('data-todate', toT);
            ...
            if (detailRow.is(':visible') && detailRow.data('active-period') === periodId) {
                ...
                icon.html('&#9654;');
                trMain.removeClass('row-open').find('td.period-col').removeClass('period-active');
                return;
            }
            detailRow.data('active-period', periodId);
            // Đánh dấu ô/kỳ đang xem
            trMain.addClass('row-open').find('td.period-col').removeClass('period-active');
        ...
        // Sửa tay ngày → bỏ chọn preset để kỳ so sánh tính theo khoảng thực nhập
        ...
        $('#ec_search_form').on('submit', function () {
            $('#btnSearch').val('⏳ Đang tải...').css('pointer-events', 'none').css('opacity', '0.7');
        });
        ...
        $(document).on('click', '.show-bk-list', function (e) {
            ...
            var prefix = statusFilter === 'completed' ? 'BK hoàn tất' : (statusFilter === 'reference' ? 'BK tham khảo' : 'Tất cả BK');
            $('#mock-modal-title-text').text(prefix + ' - ' + country + ' (' + period + ')');
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    departure: dep, arrival: arr, scope: scope, dest_country: destCountry, grp: grp,
                    status_filter: statusFilter,
                    from_date: fromDate, to_date: toDate, for: 'getDetailsAirportStatistics'
                },
                success: function (response) { $('#mock-modal-body').html(response); },
                error: function () { $('#mock-modal-body').html('<div class="text-danger text-center p-4">Có lỗi xảy ra khi tải dữ liệu!</div>'); }
            });
        });
        $('#mock-modal-close-btn').click(function () { $('#mock-modal').hide(); });
        ...
        function buildChart(key) {
            ...
            builtCharts[key] = new Chart(canvas.getContext('2d'), {
                ...
                data: {
                    labels: d.labels,
                    datasets: [
                        { label: lb.p5, data: d.p5, backgroundColor: 'rgba(207,34,46,0.75)',  borderColor: 'rgba(207,34,46,1)',  borderWidth: 1, hidden: true },
                        { label: lb.p4, data: d.p4, backgroundColor: 'rgba(191,57,137,0.75)', borderColor: 'rgba(191,57,137,1)', borderWidth: 1, hidden: true },
                        { label: lb.p3, data: d.p3, backgroundColor: 'rgba(130,80,223,0.75)', borderColor: 'rgba(130,80,223,1)', borderWidth: 1, hidden: true },
                        { label: lb.p2, data: d.p2, backgroundColor: 'rgba(26,127,55,0.8)',   borderColor: 'rgba(26,127,55,1)',   borderWidth: 1 },
                        { label: lb.p1, data: d.p1, backgroundColor: 'rgba(227,98,9,0.8)',    borderColor: 'rgba(227,98,9,1)',    borderWidth: 1 },
                        { label: lb.p0, data: d.p0, backgroundColor: 'rgba(31,111,235,0.85)', borderColor: 'rgba(31,111,235,1)',  borderWidth: 1 }
                    ]
                },
                options: {
                    responsive: true,
                    ...
                    scales: { y: { beginAtZero: true, title: { display: true, text: 'BK hoàn tất' } } }
                }
            });
        }
        ...
        function renderRoutes(detailRow) {
            var q = (detailRow.find('.route-search').val() || '').trim().toLowerCase();
            var eligible = detailRow.find('.route-item.active-route-item').filter(function () {
                if (!q) return true;
                return (($(this).attr('data-search') || '').indexOf(q) !== -1);
            });

            detailRow.find('.route-item').hide();
            eligible.each(function (i) { $(this).find('td:first').text(i + 1); });
            ...
            function renderPagination(page) {
                ...
                for (var i = start; i <= end; i++) html += '<li class="page-item ' + (i === page ? 'active' : '') + '"><a class="page-link" href="#" data-page="' + i + '">' + i + '</a></li>';
            ...
            if (wrapper.length) {
                wrapper.off('click', '.page-link').on('click', '.page-link', function (e) {
                    e.preventDefault();
