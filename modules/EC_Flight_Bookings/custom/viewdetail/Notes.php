...
trait NotesTrait {
	private function renderNoteMessageRows() {
		...
		$typemap = [
			...
			'remind' => 'Nhắc giờ bay',
			'support' => 'Hỗ trợ KH',
			'check_debt' => 'KT công nợ',
			'check_journey' => 'Checkin',
		];

		// Single query joining notes to their working-process row so the KPI
		// action type is resolved inline (no separate per-booking KPI query).
		$sql = "SELECT
				n.id AS detail_id,
				n.description,
				n.date_entered,
				...
				u.id AS user_id,
				wp.paid AS wp_paid,
				wp.called AS wp_called,
				wp.recheck AS wp_recheck,
				wp.recall AS wp_recall,
				wp.remind AS wp_remind,
				wp.check_debt AS wp_check_debt,
				wp.support AS wp_support
			FROM notes n
				LEFT JOIN users u ON u.id = n.created_by AND u.deleted = 0
				LEFT JOIN ec_working_process wp ON wp.id = n.working_process_id AND wp.deleted = 0
		...
		while ($row = $this->bean->db->fetchByAssoc($res)) {
			...
			// Derive the KPI action type from the joined working-process fields.
			// $typemap keys are the action fields in priority order.
			$t = null;
			foreach ($typemap as $field => $label) {
				if (!empty($row['wp_' . $field])) {
					$t = $field;
					break;
				}
			}
