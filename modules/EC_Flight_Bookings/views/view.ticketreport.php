...
class Viewticketreport extends SugarView
{
    ...
    function display()
    {
        if (ACLController::checkAccess('EC_Flight_Bookings', 'edit', true)) {
            ...
            $smartyCont->display('modules/EC_Flight_Bookings/tpls/view_ticket_report.tpl');
        } else {
    ...
    function airlineCodeQuery($sql_search, $sql_role, $post_fdate, $post_tdate)
    {
        ...
        while ($row = $db->fetchByAssoc($res)) {
            ...
            $html .= '<td style="text-align:left;">' . $row['airline_code'] . '</td>';
    ...
    function bookingQuery($sql_search, $sql_role, $post_fdate, $post_tdate, $condition_arr = array())
    {
        ...
        while ($row = $db->fetchByAssoc($res)) {
            ...
            if ($this->_is_allow_recheck) {
                ...
                if ($row['parent_type'] == 'EC_Flight_Bookings') {
                    ...
                    $html .= '<input style="vertical-align: middle;" type="checkbox" class="booking-ids" data-triptype="' . $row['flight_type'] . '" data-mobile="' . $row['contact_mobile'] . '" id="' . $row['parent_id'] . '" value="' . $row['parent_id'] . '">';
                }
                $html .= '</td>';
            }
            ...
            $html .= '<td class="text-center"><a target="_blank" title="Xem chi tiết" href="index.php?module=' . $row['parent_type'] . '&action=DetailView&record=' . $row['parent_id'] . '">' . $row['parent_name'] . '</a></td>';

            $html .= '<td class="text-center">' . format_number($row['total_quantity']) . '</td>';

            $html .= '<td class="text-start">' . $row['booking_description'] . ($row['not_from_web'] ? (!empty($row['booking_description']) ? '<br>' : '') . '<b>(Tạo bởi booker)</b>' : '') . '</td>';

            $html .= '<td class="text-end">' . format_number($row['subtotal_amount']) . '</td>';

            $html .= '<td class="text-end">' . format_number($row['total_bought_price']) . '</td>';

            $html .= '<td class="text-end">' . format_number($profit_amount) . '</label></td>';
            ...
            if ($row['parent_type'] == 'EC_Flight_Bookings') {
                ...
                $html .= '<td class="text-end">' . format_number($row['receipt_amount']) . '</td>';
            }
            ...
            $html .= '<td class="text-start hide-mobile row-employees"><a href="index.php?module=Employees&return_module=Employees&action=DetailView&record=' . $user_id_detail . '" target="_bank" title="Xem chi tiết nhân viên ' . $user_list[$row['user_id']] . '">' . $user_list[$row['user_id']] . '</a></td>';

            // $html .= '<td class="text-center">' . $this->getRecheckInfo($row['parent_id']) . '<div class="rc-message"></div></td>';
            $html .= '<td class="text-center">' . $row['bk_date_entered'] . '</td>';
            $html .= '<td class="text-center">' . $row['bk_date_ticket_issue'] . '</td>';
        ...
        $html .= '<tr class="footer-tr">
            ...
            <td class="text-center fw-semibold color-red total_profit">' . format_number($total_profit) . '</td>
            <td class="text-center fw-semibold color-red total_receipt">' . format_number($total_receipt) . '</td>
            <td class="employees hide-mobile">&nbsp;</td>
            <td class="date_created">&nbsp;</td>
            <td class="date_issue">&nbsp;</td>
        </tr>';
