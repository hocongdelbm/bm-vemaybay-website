...
class EC_Flight_BookingsViewDetail extends ViewDetail
{
	...
	private $list_website_new_baggage = ['557d4a5b-27ce-5cb1-4531-5800ab9ed31d', '2b2c93b3-e916-113c-29bc-5b4c6de75db4'];
	...
	function populateLinePassengers($add_type)
	{
		...
		while ($row = $this->bean->db->fetchByAssoc($res)) {
			...
			if (empty($row['add_type']) || !empty($row['luggage_price']) && $add_type == '0' || $add_type == 1 && $row['add_type'] == '1') {
				if(in_array($this->bean->created_by, $this->list_website_new_baggage)) {
					$rowBagHTML = '';
					...
					$html .= '<tr class="psg-line luggage" '. (empty($rowBagHTML) ? 'style="display:none;"' : '') .'>
						<td data-label="Hành lý ký gửi" class="text-center bg-yellow align-middle">
