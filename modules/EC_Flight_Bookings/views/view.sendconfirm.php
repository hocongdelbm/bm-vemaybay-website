...
class Viewsendconfirm extends SugarView {
	...
	function sendConfirm() {
		...
		$pax_infos 			= $this->getPassengerInfo($this->bean->id, $this->bean->flight_type);
	...
	public function getPassengerInfo($booking_id, $flight_type) {
		...
		while($row = $db->fetchByAssoc($res)){
			...
			// Baggage inbound
			if($flight_type == '0') {
				$bagIn = $this->bean->getBaggageInfoByData([
