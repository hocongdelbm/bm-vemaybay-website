...
class Viewclientphonetcb extends SugarView
{
    ...
    function display()
    {
        ...
        // Get the source from GET parameter (defaults to 'timchuyenbay.com' if not set)
        $source = isset($_GET['source']) ? $_GET['source'] : 'timchuyenbay.com';
        $smarty->assign('source', $source);
        ...
        if (!empty($bookingIds)) {
            ...
            foreach ($data as &$entry) {
                if (!empty($entry['booking_id']) && isset($bookingMap[$entry['booking_id']])) {
                    ...
                    $entry['booking_name'] = '';  // If no name found
                }
            }
        }
    ...
    function getDataFromTicketWebsite($source, $fromDate, $toDate, $offset = 0, $limit = 50, $isUsed = 0)
    {
        ...
        curl_setopt_array($curl, [
            ...
            CURLOPT_TIMEOUT => 30, // Increase timeout if necessary
            CURLOPT_FOLLOWLOCATION => true,
            CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
            CURLOPT_CUSTOMREQUEST => 'POST',
            CURLOPT_HTTPHEADER => [
                'Api-Key: 4F3yBy83DIRuHaFp6e@alBkS-sb3T3)kvZ8$-MqM',  // Add your actual API key here
    ...
    // function getDataFromTicketWebsite($source, $fromDate, $toDate, $offset = 0, $limit = 50, $isUsed = 0)
    // {
    //     // Prepare the URL based on the source
    //     $url = '';
    //     if ($source === 'timchuyenbay.vn') {
    //         $url = 'https://timchuyenbay.vn/api';
    //     } elseif ($source === 'dailyve.net') {
