...
class Viewbookingqtyreport extends SugarView
{
	function display()
	{
		...
		if (
			...
			|| $user_id == '9f381038-99c2-7515-938f-558939fee19a' // tuananh
			|| $user_id == '37cd4853-721c-9808-af64-5600c8835d03' // ngandtk
			|| $user_id == '9eb0f65f-a9f6-65bb-1985-637ca8511491' // trinhnvd
		) {
			$smarty = new Sugar_Smarty();
			$this->generateSearchResult($smarty);
			$this->populateRadioBtn($smarty);
			$smarty->display('modules/EC_Flight_Bookings/tpls/view_bookingqytreport.tpl');
		} else {
			header("Location: index.php?module=EC_Flight_Bookings&action=Error&error_string=" . urlencode("Bạn không có quyền xem báo cáo này. Vui lòng liên hệ quản trị viên để được cấp quyền."));
			exit;
		}
	}
	...
	function genBKSale($from_date, $to_date, $type = 0)
	{
		...
		$sql = '
			...
			FROM (
				SELECT
					...
					-- SUM(IFNULL((SELECT COUNT(DISTINCT parent_id) FROM ec_flight_bookings_audit WHERE parent_id = bk.id AND field_name = "contact_name" AND before_value_string = "Panda Po"), 0) + (SELECT COUNT(DISTINCT id) FROM ec_flight_bookings WHERE id = bk.id AND contact_name = "Panda Po")) AS my_bk,
					-- SUM(IFNULL((SELECT COUNT(DISTINCT parent_id) FROM ec_flight_bookings_audit WHERE parent_id = bk.id AND field_name = "contact_name" AND before_value_string = "Panda Po" AND bk.booking_status IN (3, 7, 8)), 0) + (SELECT COUNT(DISTINCT id) FROM ec_flight_bookings WHERE id = bk.id AND contact_name = "Panda Po" AND booking_status IN (3, 7, 8))) AS com_my_bk,
				...
				WHERE u.title = "Bot" 
					-- AND bk.date_entered >= "' . date("Y-m-d H:i:s", strtotime($from_date) - 7 * 60 * 60) . '"
					-- AND bk.date_entered <= "' . date("Y-m-d 16:59:59", strtotime($to_date)) . '"
				...
				UNION
				SELECT 
					IF(u.title = "Bot", u.last_name, "Booking chưa xác định") AS last_name,
				...
				FROM ec_flight_bookings bk
					LEFT JOIN users u ON bk.created_by = u.id AND u.deleted = 0
				WHERE
					-- bk.date_entered >= "' . date("Y-m-d H:i:s", strtotime($from_date) - 7 * 60 * 60) . '"
					-- AND bk.date_entered <= "' . date("Y-m-d 16:59:59", strtotime($to_date)) . '"
		...
		$html 	= '<tbody><form id="booking_search" name="search_form" method="POST" action="index.php?module=EC_Flight_Bookings&action=ListView" target="_blank">';
		$i 		= 0;
		...
		while ($row = $this->bean->db->fetchByAssoc($res)) {
			...
			$total_sale_qty 	+= ($row['bk_confirmed'] + $row['bk_printed'] + $row['bk_completed'] - $row['com_inter_ticket_qty']);
			$total_sale_ticket 	+= ($row['total_ticket'] - $row['com_inter_ticket']);
			...
			$total_ticket_prior_bk += ($row['com_prior_ticket'] - $row['com_inter_ticket']);
			$total_sale_prior_bk += ($row['prior_bk_sales'] - $row['bk_inter_ticket_sales']);
			...
			if ($row['my_bk'] == 0) {
				...
				$my_bk = format_number($row['my_bk'] - $row['bk_inter_ticket']);
			}
			if ($row['com_my_bk'] == 0) {
				$com_my_bk = 0;
			} else {
				$com_my_bk = format_number($row['com_my_bk'] - $row['com_inter_ticket_qty']);
			}
			...
			if ($row['prior_bk'] == 0) {
				...
				$prior_bk = format_number($row['prior_bk'] - $row['bk_inter_ticket']);
			}

			if ($row['com_prior_bk'] == 0) {
				$com_prior_bk = 0;
			} else {
				$com_prior_bk = format_number($row['com_prior_bk'] - $row['com_inter_ticket_qty']);
			}
			$prior_bk_sales = format_number($row['prior_bk_sales'] - $row['bk_inter_ticket_sales']);
			
			$html .= '
				<tr>
					<td class="text-center fw-semibold">' . ($i + 1) . '</td>
					<td class="text-center fw-semibold">' . $name . '</td>

					<td colspan="2" class="text-start">
						<div class="d-flex align-items-center justify-content-between">
							<div class="total text-start">' . format_number($row['total_sales'] - $row['bk_inter_ticket_sales']) . '</div>
							<div class="total_percent text-end">&nbsp;&nbsp;($SALE_PER' . $row['user_id'] . ')</div>
						</div>
					</td>

					<td class="text-center total_ticket">' . $total_ticket . '</td>
					<td class="text-center fw-semibold color-blue">' . format_number($row['bk_confirmed'] + $row['bk_printed'] + $row['bk_completed'] - $row['com_inter_ticket_qty']) . '</td>
					
					<td colspan="2">
						<div class="d-flex align-items-center justify-content-between">
							<div class="total_percent text-start">(' . format_number(($row['bk_confirmed'] + $row['bk_printed'] + $row['bk_completed']) / $row['total'] * 100) . '%)</div>
							<div class="total text-end color-red fw-semibold">&nbsp;&nbsp;' . format_number($row['total'] - $row['bk_inter_ticket']) . '</div>
						</div>
					</td>
					...
					<td colspan="2">
						<div class="d-flex align-items-center justify-content-between">
							<div class="com_prior_ticket text-start">(' . format_number($row['com_prior_ticket'] - $row['com_inter_ticket']) . '&nbsp;vé)</div>
					...
					<td class="text-end">' . format_number($row['bk_completed']) . '</td>
					<td class="text-end">' . round($row['bk_completed'] / $row['total'] * 100, 1) . '%</td>
					
					<td class="text-end">' . format_number($row['bk_called']) . '</td>
					<td class="text-end">' . round($row['bk_called'] / $row['total'] * 100, 1) . '%</td>
					
					<td class="text-end">' . format_number($row['bk_cancelled']) . '</td>
					<td class="text-end">' . round($row['bk_cancelled'] / $row['total'] * 100, 1) . '%</td>
				</tr>';
			$i++;

			// <td class="text-end">'.format_number($row['bk_confirmed']).'</td>
			// <td class="text-end">'.round($row['bk_confirmed'] / $row['total'] * 100, 1).'%</td>
		...
		// $html .= '<input type="hidden" name="booking_status_advanced[]" id="booking_status_advanced4" value="1"/>';
		// $html .= '<input type="hidden" name="booking_status_advanced[]" id="booking_status_advanced5" value="2"/>';
		// $html .= '<input type="hidden" name="booking_status_advanced[]" id="booking_status_advanced6" value="4"/>';
		// $html .= '<input type="hidden" name="booking_status_advanced[]" id="booking_status_advanced7" value="6"/>';
		...
		$html .= '<tfoot>
			<tr class="footer-tr">
				<td colspan="2" class="text-center fw-semibold">Tổng cộng</td>
				<td colspan="2" class="text-end color-red fw-semibold">
					<div class="d-flex align-items-center justify-content-between">
						<div class="total_sale">' . format_number($total_sale - $total_inter_ticket_sales) . '</div>
						<div class="total_sale_percent">&nbsp;&nbsp;(100%)</div>
					</div>
				</td>
				...
				<td colspan="2" class="text-end total__booking-today"><span class="show_detail_total show_detail" title="Chi tiết booking trong ngày">' . format_number($total - $total_inter_bk) . '</span></td>
				<td class="text-end total_my_bk-today">' . format_number($total_my_bk - $total_inter_bk) . '&nbsp;/&nbsp;' . format_number($total_com_mybk - $total_com_inter_ticket_qty) . '</td>

				<td class="text-end total_prior_bk-today">' . format_number($total_prior_bk - $total_inter_bk) . '&nbsp;/&nbsp' . format_number($total_comprior - $total_com_inter_ticket_qty) . '</td>
				...
				<td style="text-align: right; background-color: #BBD6B8; ">' . format_number($total_called) . '</td>
				<td style="text-align: right; background-color: #BBD6B8; ">' . round($total_called / $total * 100, 1) . '%</td>
