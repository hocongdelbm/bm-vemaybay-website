...
<table width="100%" cellpadding="0" cellspacing="1" border="0" class="actionsContainer">
	<tr>
		<td>
			<input title="{$APP.LBL_SAVE_BUTTON_TITLE}" accessKey="{$APP.LBL_SAVE_BUTTON_KEY}" class="btn btn-primary" id="ConfigureSettings_save_button" type="submit"  name="save" value="  {$APP.LBL_SAVE_BUTTON_LABEL}  " >
			<input title="{$MOD.LBL_SAVE_BUTTON_TITLE}"  id="ConfigureSettings_restore_button" class="btn btn-primary"  type="submit" name="restore" value="  {$MOD.LBL_RESTORE_BUTTON_LABEL}  " >
			<input title="{$MOD.LBL_CANCEL_BUTTON_TITLE}" id="ConfigureSettings_cancel_button" onclick="document.location.href='index.php?module=Administration&action=index'" class="btn btn-danger"  type="button" name="cancel" value="  {$APP.LBL_CANCEL_BUTTON_LABEL}  " > 
		</td>
	</tr>
</table>

<div class="setup-system-wrap">
	<div class="box-section">
		<table border="0" cellspacing="1" cellpadding="0" class="table-config table-edit table-config table-editview__configurator">
			<tr>
			...
			<tr>
				...
				<td>
					<input class="box-input" type='text' size='4' id='ConfigureSettings_list_max_entries_per_page' name='list_max_entries_per_page' value='{$config.list_max_entries_per_page}'>
				</td>
				<td scope="row">{$MOD.LIST_ENTRIES_PER_SUBPANEL}: </td>
				<td>
					<input class="box-input" type='text' size='4' id='ConfigureSettings_list_max_entries_per_subpanel' name='list_max_entries_per_subpanel' value='{$config.list_max_entries_per_subpanel}'>
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td width='35%'>
					<input type='text' class="box-input" name='system_name' value='{$settings.system_name}'>
				</td>
				<td scope="row" width='15%' nowrap>{$MOD.LBL_MIN_AUTO_REFRESH_INTERVAL} &nbsp;{sugar_help text=$MOD.LBL_MIN_AUTO_REFRESH_INTERVAL_HELP} </td>
				<td width='35%'>
				    <select class="box-select" name='dashlet_auto_refresh_min' id='dashlet_auto_refresh_min'>{$AUTO_REFRESH_INTERVAL_OPTIONS}</select>
				</td>
		    </tr>
		    <tr>
		    ...
		    <tr>
			   ...
			   <td  width='35%'>
				  ...
				  <input type='text' class="box-input" id='company_logo' name='company_logo' style="display:none">
			   </td>
		    </tr>
		    <tr>
				  <td scope="row">{$MOD.LBL_LEAD_CONV_OPTION}:&nbsp;{sugar_help text=$MOD.LEAD_CONV_OPT_HELP}</td>
				  <td> <select class="box-select" name="lead_conv_activity_opt">{$lead_conv_activities}</select></td>
	...
	<table width="100%" border="0" cellspacing="1" cellpadding="0" class="table-config table-edit">
		...
		<div id="proxy_config_display" style='display:{$PROXY_CONFIG_DISPLAY}'>
			...
			<td width="35%" ><input type="text" class="box-select" id="proxy_host" name="proxy_host" size="25"  value="{$settings.proxy_host}" tabindex='1' ></td>
			<td width="15%" scope="row">{$MOD.LBL_PROXY_PORT}<span class="required">{$APP.LBL_REQUIRED_SYMBOL}</span></td>
			<td width="35%" ><input type="text" class="box-select" id="proxy_port" name="proxy_port" size="6"  value="{$settings.proxy_port}" tabindex='1' ></td>
			</tr><tr>
	...
	<table width="100%" border="0" cellspacing="1" cellpadding="0" class="table-config table-edit">
		...
		<tr>
			...
			<td  >
				<input type='text' class="box-input" size='5' name='slow_query_time_msec' value='{$config.slow_query_time_msec}'>
			</td>
		...
		<tr>
			...
			<td  >
				<input type='text' class="box-input" size='8' name='upload_maxsize' value='{$config.upload_maxsize}'>
			</td>
		...
		<tr>
		   ...
		   <td scope="row">{$MOD.LBL_IMPORT_MAX_RECORDS} {sugar_help text=$MOD.LBL_IMPORT_MAX_RECORDS_HELP}</td>
			<td >
				<input type='text' class="box-input" size='4' name='import_max_records_total_limit' value='{$config.import_max_records_total_limit}'>
			</td>
	...
	{if $logger_visible}
		...
		<tr>
			...
			<td valign='middle'>
				<input type='text' class="box-input" name = 'logger_file_name'  value="{$config.logger.file.name}">
				<div><span class="small">{$APP.LBL_LOGGER_VALID_FILENAME_CHARACTERS}</span></div>
			</td>
			<td scope="row">{$MOD.LBL_LOGGER_FILE_EXTENSION}</td>
			<td><input class="box-input" name ="logger_file_ext" type="text" size="5" value="{$config.logger.file.ext}"></td>
			<td scope="row">{$MOD.LBL_LOGGER_FILENAME_SUFFIX}</td>
			<td><select class="box-select" name = "logger_file_suffix" selected='{$config.logger.file.suffix}'>{$filename_suffix}</select></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td> <select class="box-select" name="logger_level">{$log_levels}</select></td>
			...
			<td> <input class="box-input" name="logger_file_maxLogs" value="{$config.logger.file.maxLogs}"></td>
		</tr>
	{/if}
		<tr>
...
<table width="100%" cellpadding="0" cellspacing="1" border="0" class="actionsContainer">
	<tr>
		<td>
			<input title="{$APP.LBL_SAVE_BUTTON_TITLE}" class="btn btn-primary"  type="submit" name="save" value="  {$APP.LBL_SAVE_BUTTON_LABEL}  " class="btn btn-save"/>
			<input title="{$MOD.LBL_SAVE_BUTTON_TITLE}"  class="btn btn-primary"  type="submit" name="restore" value="  {$MOD.LBL_RESTORE_BUTTON_LABEL} " />
			<input title="{$MOD.LBL_CANCEL_BUTTON_TITLE}" onclick="document.location.href='index.php?module=Administration&action=index'" class="btn btn-danger"  type="button" name="cancel" value="  {$APP.LBL_CANCEL_BUTTON_LABEL}  " />
		</td>
	</tr>
</table>
