...
eoq;
    ...
    public function getMassUpdateForm(
        ...
        $html = "<div id='massupdate_form' class='box-section' style='display:none;'><table cellpadding='0' cellspacing='0' border='0' class='formHeader h3Row'><tr><td nowrap><h4 class='sub-admin__title'>" . $app_strings['LBL_MASS_UPDATE'] . "</h4></td></tr></table>";
        $html .= "<div id='mass_update_div'><table cellpadding='0' cellspacing='1' border='0' class='table-edit table-mass-update__in-include' id='mass_update_table'>";

        $even = true;

        if ($this->sugarbean->object_name == 'Contact') {
            $html .= "<tr><td width='15%' scope='row'>$lang_sync</td><td width='35%' class='dataField'><select class='box-select' name='Sync'><option value=''>{$GLOBALS['app_strings']['LBL_NONE']}</option><option value='false'>{$GLOBALS['app_list_strings']['checkbox_dom']['2']}</option><option value='true'>{$GLOBALS['app_list_strings']['checkbox_dom']['1']}</option></select></td>";
            $even = false;
        } else {
        ...
        if (in_array($this->sugarbean->object_name, ['Contact', 'Account', 'Lead', 'Prospect'])) {
            $optOutPrimaryEmail =
                ...
                . "<select class='box-select' name='optout_primary'>"
            ...
            if ($configurator->isConfirmOptInEnabled() || $configurator->isOptInEnabled()) {
                $optInPrimaryEmail =
                    ...
                    . '</td>'
                    . '<td>'
                    . "<select class='box-select' name='optin_primary'>"
                    . "<option value=''>{$app_strings['LBL_NONE']}</option>"
                    . "<option value='false'>{$GLOBALS['app_list_strings']['checkbox_dom']['2']}</option>"
                    . "<option value='true'>{$GLOBALS['app_list_strings']['checkbox_dom']['1']}</option>"
                    . "</select>"
        ...
        $html .= "<table cellpadding='0' cellspacing='0' border='0' width='100%'><tr><td class='buttons'><div class='d-flex align-items-center gap-2'><input onclick='return sListView.send_mass_update(\"selected\", \"{$app_strings['LBL_LISTVIEW_NO_SELECTED']}\")' type='submit' id='update_button' name='Update' value='{$lang_update}' class='btn btn-primary'><input onclick='javascript:toggleMassUpdateForm();' type='button' id='cancel_button' name='Cancel' value='{$GLOBALS['app_strings']['LBL_CANCEL_BUTTON_LABEL']}' class='btn btn-danger'></div>";
        ...
        if (
            ...
            $html .= <<<eoq
			<input type='button' name='archive' value="{$lang_archive}" class='btn btn-primary' onClick='setArchived();'>
			<input type='hidden' name='ie_assigned_user_id' value="{$current_user->id}">
...
EOJS;
        ...
        $html = "<div id='massupdate_form' style='display:none;'><table width='100%' cellpadding='0' cellspacing='0' border='0' class='formHeader h3Row'><tr><td nowrap><h4 class='sub-admin__title'>" . $app_strings['LBL_MASS_UPDATE'] . "</h4></td></tr></table>";
...
<td>
    ...
    <tr>
        <td valign='top'>
            <select class='box-select' name='{$field['type_name']}' id='mass_{$field['type_name']}'>
                $types
            </select>
        </td>
        <td valign='top'>
			<input class='box-input' name='{$field['id_name']}' id='mass_{$field['id_name']}' type='hidden' value=''>
			<input class='box-input' name='parent_name' id='mass_parent_name' class='sqsEnabled' autocomplete='off'
                type='text' value=''>
...
EOHTML;
    ...
    public function addInputType($displayname, $varname)
    {
        ...
        $html = <<<EOQ
	<td scope="row" width="20%">$displayname</td>
	<td class='dataField' width="30%"><input class='box-input' type="text" name='$varname' size="12" id='{$varname}' maxlength='10' value=""></td>
...
<td width='35%' class='dataField'>
    <input class='box-input' name='{$varname}' id='mass_{$varname}' class='sqsEnabled' autocomplete='off' type='text' value=''>
    <input class='box-input' name='{$id_name}' id='mass_{$id_name}' type='hidden' value=''>&nbsp;
    <input title='{$app_strings['LBL_SELECT_BUTTON_TITLE']}'
        type='button' class='btn btn-primary' value='{$app_strings['LBL_SELECT_BUTTON_LABEL']}' name='button'
...
EOHTML;
    ...
    public function addAccountID($displayname, $varname, $id_name = '')
    {
        ...
        $html = '<td scope="row">' . $displayname . " </td>\n"
            . '<td><input class="sqsEnabled box-input" type="text" autocomplete="off" id="mass_' . $varname . '" name="' . $varname . '" value="" /><input id="mass_' . $id_name . '" type="hidden" name="'
            . $id_name . '" value="" />&nbsp;<span class="id-ff multiple"><button type="button" name="btn1" class="btn btn-primary" title="'
    ...
    public function addAssignedUserID($displayname, $varname)
    {
		...
		<td>
            <div class="d-flex align-items-center gap-2">
                <input class="sqsEnabled box-input" autocomplete="off" id="mass_assigned_user_name" name='assigned_user_name' type="text" value="">
                <input id='mass_assigned_user_id' name='assigned_user_id' type="hidden" value="" />
                <span class="id-ff multiple">
                    <button id="mass_assigned_user_name_btn" title="{$app_strings['LBL_SELECT_BUTTON_TITLE']}" type="button" class="btn btn-primary" value='{$app_strings['LBL_SELECT_BUTTON_LABEL']}' name=btn1 onclick='open_popup("Users", 600, 400, "", true, false, $encoded_popup_request_data);' />
                        <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M10 18a7.952 7.952 0 0 0 4.897-1.688l4.396 4.396 1.414-1.414-4.396-4.396A7.952 7.952 0 0 0 18 10c0-4.411-3.589-8-8-8s-8 3.589-8 8 3.589 8 8 8zm0-14c3.309 0 6 2.691 6 6s-2.691 6-6 6-6-2.691-6-6 2.691-6 6-6z"></path><path d="M11.412 8.586c.379.38.588.882.588 1.414h2a3.977 3.977 0 0 0-1.174-2.828c-1.514-1.512-4.139-1.512-5.652 0l1.412 1.416c.76-.758 2.07-.756 2.826-.002z"></path></svg>
                    </button>
                </span>
            </div>
...
EOQ;
    ...
    public function addStatus($displayname, $varname, $options)
    {
        ...
        if (is_array($options)) {
            ...
            $html .= '<select class="box-select" id="mass_' . $varname . '" name="' . $varname . '">' . $options . '</select>';
        } else {
            $html .= $options;
        }
        $html .= '</td>';
    ...
    public function addStatusMulti($displayname, $varname, $options)
    {
        ...
        $html = '<td scope="row" width="15%">' . $displayname . '</td>
			 <td><select class="box-select" id="mass_' . $varname . '" name="' . $varname . '[]" size="5" MULTIPLE>' . $options . '</select></td>';
...
EOQ;
        $html = <<<EOQ
	<td scope="row" width="20%">$displayname</td>
	<td class='dataField' width="30%"><input class="box-input" onblur="parseDate(this, '$cal_dateformat')" type="text" name='$varname' size="12" id='{$varname}jscal_field' maxlength='10' value="">
...
EOQ;
        ...
        $html = <<<EOQ
		<td scope="row" width="20%">$displayname</td>
		<td class='dataField' width="30%"><input class="box-input" onblur="parseDate(this, '$cal_dateformat')" type="text" name='$varname' size="12" id='{$varname}_date' maxlength='10' value="">
		&nbsp;$javascriptend
