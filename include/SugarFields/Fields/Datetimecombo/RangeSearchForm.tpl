...
<div id="{$id}_range_div" style="{if preg_match('/^\[/', $smarty.request.{{$id_range}})  || $starting_choice == 'between'}display:none{else}display:flex{/if}; gap: 0.5rem; flex: 1;">
...
<div id="{$id}_between_range_div" style="{if $starting_choice=='between'}display:flex;{else}display:none;{/if}">
