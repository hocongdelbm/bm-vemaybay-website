...
This is a customized **SuiteCRM 7.13.3** instance for a Vietnamese travel/flight booking agency (bm.vemaybay.website). It extends the standard SuiteCRM CRM framework with ~40 custom `EC_*` modules for flight bookings, invoicing, HR, accounting, and integrations (Zalo, SMS, WebRTC, Telegram, OnePay, WinInvoice).
...
- `custom/include/helpers/api/` — Third-party API wrappers (Datacom, WinInvoice, Zalo, NextCloud, OnePay, Phương Nam)
- `custom/include/helpers/cache/` — Cache abstraction layer (Session, File drivers) — defined but not currently wired up anywhere (no call sites found)
...
- `custom/json_files/` — Runtime JSON config files (airports, airlines, Zalo data)
...
│   ├── entryOutputInvoiceClass      — Output (sales) invoice + e-invoice (WinInvoice)
