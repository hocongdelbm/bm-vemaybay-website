...
if (!empty($_SESSION['authenticated_user_id'])) {
	...
	if ($module == 'Contacts') {
		...
		$sql = "SELECT c.id, c.name, c.date_entered, c.description
				FROM calls c LEFT JOIN contacts a ON c.parent_id=a.id AND a.deleted=0
				WHERE c.deleted=0 AND c.parent_type='Contacts' AND c.assigned_user_id='" . $uid . "'
				AND DATE(c.date_entered) >= '" . date('Y-m-d', strtotime($fdate)) . "' AND DATE(c.date_entered) <= '" . date('Y-m-d', strtotime($tdate)) . "' ";
