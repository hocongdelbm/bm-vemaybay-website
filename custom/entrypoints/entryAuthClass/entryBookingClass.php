...
class entryBookingClass extends entryClass
{
    ...
    public function getUploadedDocuments($params = [])
    {
        ...
        while ($row = $db->fetchByAssoc($result)) {
            ...
            $preview_image = "Không phải là file ảnh";
            if (!empty($row['preview_image'])) {
                $preview_image = "index.php?entryPoint=download&id=" . $row['id'] . "_preview_image&type=Documents";
            }
            $documents[] = [
                'id' => $row['id'],
                ...
                'preview_image' => (!empty($row['preview_image'])) ? $preview_image : 'Không phải file ảnh',
