    ...
    if (configuration.uri && configuration.password) {
        $.ajax({
            ...
            success: function (response) {
                let always_online = ['010', '012', '888', '222'];
...
ua.on('newRTCSession', function (ev) {
    ...
    if (session) {
        if(ev.session.direction === "incoming") ev.session.terminate(); // End the call to forward
    ...
    session.on("confirmed",function() {
        ...
        // var localStream = session._connection.getLocalStreams()[0];
        // var dtmfSender = session._connection.createDTMFSender(localStream.getAudioTracks()[0]);
...
$(document).ready(function () {
    ...
    $(document).on('click', '#call-phone__circle', function () {
        if (navigator.mediaDevices) {
            ...
            .then(function(stream) {
                console.log('Thiết bị có microphone');
            })
    ...
    $(document).on('click', '.btn-voiceip-calling', function () {
        ...
        if(number.length > 0 && number != SIP_USER) {
            ...
            if(number.charAt(0) == '6') {
                ...
                callOptions.extraHeaders = ['X-Caller: GiaoNhanh']
            }
            else callOptions.extraHeaders = ['X-Caller: Travelpass']
            ...
            $.ajax({
                ...
                success: function (response) {
                    if(response.length > 0 && response != '[]') {
                        ...
                        $('#popup-inforbooking').html(info_booking);
    ...
    // Nút đóng popup cuộc gọi
    // $(document).on('click', '.voiceip-close, .voiceip-header__close', function () {
    //     $('#popup-voiceip').removeClass('show');
    //     $('#call-overlay').removeClass('opened');
    // });
