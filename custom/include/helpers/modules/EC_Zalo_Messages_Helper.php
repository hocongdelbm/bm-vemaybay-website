...
class EC_Zalo_Messages_Helper {
    public const IMAGE_FILE = [
        'excel' => EC_Zalo_Helper::IMAGE_PATH . "file-excel.jpg",
        'word' => EC_Zalo_Helper::IMAGE_PATH . "file-word.jpg",
        'powerpoint' => EC_Zalo_Helper::IMAGE_PATH . "file-powerpoint.jpg",
        'pdf' => EC_Zalo_Helper::IMAGE_PATH . "file-pdf.jpg",
        'txt' => EC_Zalo_Helper::IMAGE_PATH . "file-txt.jpg",
        'html' => EC_Zalo_Helper::IMAGE_PATH . "file-html.jpg",
        'xml' => EC_Zalo_Helper::IMAGE_PATH . "file-xml.jpg",
        'zip' => EC_Zalo_Helper::IMAGE_PATH . "file-zip.jpg",
        'rar' => EC_Zalo_Helper::IMAGE_PATH . "file-rar.jpg",
        'image' => EC_Zalo_Helper::IMAGE_PATH . "file-image.jpg",
        'default' => EC_Zalo_Helper::IMAGE_PATH . "file-default.jpg",
    ];
     ...
     * @return array
     */
    public static function get_list_recent_messages($oa_id, $last_timestamp = 0, $current_list_user = [], $limit_record = 15) {
        global $db;
     ...
     */
    public static function get_messages($oa_id, $zalo_id, $zalo_phone = '', $offset = 0, $is_get_user_info = 0, $limit_message = 10) {
        global $db;
        $result = [];
        ...
        while($row = $db->fetchByAssoc($res)) {
            ...
            if($row['quote_id'] && !empty($row['quote_id'])) {
                $row['quote_data'] = self::get_quote_message_data($row['quote_id']);
            }
        ...
        try {
            ...
            $GLOBALS['log']->warn("{$e->getMessage()} on line {$e->getLine()} in {$e->getFile()}");
        }
     ...
     * @return array
     */
    public static function get_lastest_message_user($oa_id, $zalo_id, $only_user_interaction = false) {
     ...
     * @return array
     */
    public static function get_quote_message_data($quote_id) {
        ...
        if($result['parent_type'] == 'call') {
            ...
            elseif($row['type'] == 'file') {
                ...
                $result['thumbnail'] = self::IMAGE_FILE[$file_type] ?? '';
     ...
     * @return string
     */
    public static function map_sub_type($event_name) {
        $arr = [
            'user_send_text'            => 'text',
     ...
     * @return int Cost
     */
    public static function handle_quota_and_calculate_cost($data, $zalo_id = '', $oa_id = '') {
        $cost = 0;
            ...
            try {
                ...
                switch ($quotaData['quota_type']) {
                    ...
                    case 'reward_quota': // Tin gửi ra là tin nằm trong hạn mức Redeem code
                        ...
                        break;
                    case 'zbs';
                        if(isset($data['sending_mode']) && (int)$data['sending_mode'] == 1) {
                            // Get oa's current quota from db
                            $quotaInfo = $db->getOne("SELECT IFNULL(quota_info, '') FROM ec_zalo WHERE id = '{$oa_id}' AND deleted = 0") ?? '';
                            $isUpdated = false;
                ...
                $GLOBALS['log']->warn("{$th->getMessage()} on line {$th->getLine()} in {$th->getFile()}");
            }
        }
        else $cost = 55;
        return $cost;
