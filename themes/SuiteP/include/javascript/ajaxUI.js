...
SUGAR.ajaxUI = {
          ...
          try {
               ...
               if (typeof (r.responseTime) != 'undefined') {
                    ...
                    if (rt.length > 0) {
                         ...
                         $("#logo").attr("title", logoStats.replace(/[\d]+\.[\d]+/, r.responseTime)).tipTip({
                              maxWidth: "auto",
                              edgeOffset: 10
                         });
                    }
     ...
     loadContent: function (url, params) {
          if (YAHOO.lang.trim(url) != "") {
               ...
               if (/^index.php?(([A-Z]|[a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+[%=+#&\[\]\.]*)+/i.test(testUrl) === false) {
                    throw "Invalid url";
               }
     ...
     submitForm: function (formname, params) {
          ...
          if (SA.canAjaxLoadModule(form.module.value) && typeof (YAHOO.util.Selector.query("input[type=file]", form)[0]) == "undefined" && /action=ajaxui/.exec(window.location)) {
     ...
     print: function () {
          var url = YAHOO.util.History.getBookmarkedState('ajaxUILoc');
          SUGAR.util.openWindow(url + '&print=true', 'printwin', 'menubar=1,status=0,resizable=1,scrollbars=1,toolbar=0,location=1');
     },
     showLoadingPanel: function () {
          if (!SUGAR.ajaxUI.loadingPanel) {
               ...
               SUGAR.ajaxUI.loadingPanel.setBody('<div id="loadingPage"><div class="spinner"></div></div>');
