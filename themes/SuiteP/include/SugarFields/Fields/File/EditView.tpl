...
{{/if}}
{if !$noChange}
<!-- <input type='button' class='btn btn-danger tesss' id='remove_button' value='{$APP.LBL_REMOVE}' onclick='SUGAR.field.file.deleteAttachment("{{$idName}}","{{$vardef.docType}}",this);'> -->
<input type='button' class='btn btn-danger tesss' id='remove_button' value='{$APP.LBL_REMOVE}' onclick="SUGAR.field.file.deleteAttachment('{{$idName}}', '{{$vardef.docType}}', this);">
{/if}
</span>
{if !$noChange}
