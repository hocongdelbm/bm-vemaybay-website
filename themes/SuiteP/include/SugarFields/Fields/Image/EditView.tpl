...
<span id="{{$idName}}_old" style="display:{if !$showRemove}none;{/if}">
  ...
  <a href="index.php?entryPoint=download&id={$fields.{{$vardef.fileId}}.value}_{{if empty($displayParams.idName)}}{{sugarvar key='name'}}{{else}}{{$displayParams.idName}}{{/if}}&type={{$vardef.linkModule}}&time={$fields.date_modified.value}" class="tabDetailViewDFLink editview in theme type image">
    ...
    {{/if}}
    {if !$noChange}
        <input type='button' class='btn btn-danger' id='remove_button' value='{$APP.LBL_REMOVE}' onclick='SUGAR.field.file.deleteAttachment("{{$idName}}","{{$vardef.docType}}",this);'>
    {/if}
</span>
{if !$noChange}
<span id="{{$idName}}_new" style="display:{if $showRemove}none;{/if}">
<input type="hidden" name="{{$idName}}_escaped">
<input onchange="demoImage(this)" id="{{$idName}}_file" name="{{$idName}}_file"
...
<img id="prevImagePhoto_new" style="max-width: 100%;" src="#" alt="" style="display: none;">
...
<button type="button" name="{{$idName}}_remoteClearBtn" id="{{$idName}}_remoteClearBtn" tabindex="{{$tabindex}}" title="{$APP.LBL_CLEAR_BUTTON_TITLE}" class="btn btn-danger lastChild" value="{$APP.LBL_CLEAR_BUTTON_LABEL}" onclick="SUGAR.field.file.clearRemote('{{$idName}}'); return false;">
