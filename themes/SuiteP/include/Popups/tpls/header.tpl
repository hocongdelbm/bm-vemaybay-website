...
<table cellpadding="0" cellspacing="0" border="0" class="table-search-form-header__theme">
<tr>
<td>
<form action="index.php" method="post" name="popup_query_form" id="popup_query_form" class="box-section">
...
<div class="d-flex align-items-center gap-2">
	<input type="submit" name="button" class="btn btn-primary" id="search_form_submit" title="{$APP.LBL_SEARCH_BUTTON_TITLE}" value="{$APP.LBL_SEARCH_BUTTON_LABEL}" />
	<!-- <input type="reset" onclick="SUGAR.searchForm.clear_form(this.form); return false;" class="btn btn-danger" id="search_form_clear" title="{$APP.LBL_CLEAR_BUTTON_TITLE}" value="{$APP.LBL_CLEAR_BUTTON_LABEL}"/> -->
	<input type="reset" onclick="SUGAR.searchForm.clear_form(this.form); SUGAR.ajaxUI.submitForm(this.form); return false;" class="btn btn-danger" id="search_form_clear" title="{$APP.LBL_CLEAR_BUTTON_TITLE}" value="{$APP.LBL_CLEAR_BUTTON_LABEL}"/>
</div>
</td>
<td align='right'></td>
</tr>
...
{{if isset($popupMeta)}}
<div id='addformlink'>
<input type="button" name="showAdd" class="btn btn-primary" value="{$popupMeta.create.createButton}" onclick="toggleDisplay('addform');" />
</div>
{{/if}}
...
{$ADDFORMHEADER}
<table cellpadding="0" cellspacing="0" border="0" class="table-addformheader__theme">
<tr>
<td>
...
<input type='hidden' name='Leads_LEAD_offset' value=''><input type='hidden' name='saved_associated_data' value=''><input type='hidden' name='module' value='{$module}'><input type='hidden' name='action' value='Popup'><input type='hidden' name='return_module' value='{$module}'><input type='hidden' name='return_action' value='Popup'><input type='hidden' name='hide_clear_button' value='true'><input type='hidden' name='current_query_by_page' value='{$current_query}'>
	...
	{$multiSelectData}
	<input class="btn btn-primary btn-select__theme" type="button" id="MassUpdate_select_button" value='{$APP.LBL_SELECT_BUTTON_LABEL}' onclick="send_back_selected('{$module}',document.MassUpdate,'mass[]','{$APP.ERR_NOTHING_SELECTED}');">
{{/if}}