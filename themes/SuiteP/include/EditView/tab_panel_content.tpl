        ...
        {{foreach name=rowIteration from=$panel key=row item=rowData}}
            ...
            {{foreach name=colIteration from=$rowData key=col item=colData}}
                ...
                {{if $smarty.foreach.colIteration.total > 1 && $colData.colspan != 3}}
                    <div class="col-12 col-sm-6 col-md-6 edit-view-row-item" data-field="{{$colData.field.name}}">
                {{else}}
                    <div class="col-12 col-sm-12 col-md-12 edit-view-row-item" data-field="{{$colData.field.name}}">
                {{/if}}
                    <div class="row g-0 pb-2-5 align-items-start">
                ...
                {{foreach name=fieldIteration from=$colData key=field item=subField}}
                    ...
                    {{if $fieldCount < $smarty.foreach.colIteration.total && $addressCount < 1 && !empty($colData.field.name) && empty($colData.field.hideIf)}}
                        ...
                        {*<!-- LABEL -->*}
                        {{if $smarty.foreach.colIteration.total > 1 && $colData.colspan != 3}}
                            <div class="col-4 col-sm-4 col-md-4 label" data-label="{{$fields[$colData.field.name].vname}}">
                        {{else}}
                             <div class="col-4 col-sm-2 col-md-2 label" data-label="{{$fields[$colData.field.name].vname}}">
                        {{/if}}
                                ...
                                {{elseif isset($colData.field.label)}}
                                    ...
                                    <span class="label colData-field-label">{$label|strip_semicolon}:</span>
                                {{elseif isset($fields[$colData.field.name])}}
                                    {capture name="label" assign="label"}{sugar_translate label='{{$fields[$colData.field.name].vname}}' module='{{$module}}'}{/capture}
                                    <span class="label colData-field-label-name">{$label|strip_semicolon}:</span>
                                {{else}}
                                    &nbsp;
                                {{/if}}
                        ...
                        {{if !empty($colData.field.hideLabel) && $colData.field.hideLabel == true && $colData.colspan != 3}}
                            {{assign var="fieldClasses" value="col-12 col-sm-12"}}
                        {{else}}
                            {{assign var="fieldClasses" value="col-8 col-sm-8"}}
                        {{/if}}

                        <div class="{{$fieldClasses}} edit-view-field position-relative {{if $inline_edit && !empty($colData.field.name) && ($fields[$colData.field.name].inline_edit == 1 || !isset($fields[$colData.field.name].inline_edit))}}inlineEdit{{/if}}" type="{{$fields[$colData.field.name].type}}" field="{{$fields[$colData.field.name].name}}" {{if $colData.colspan}}colspan='{{$colData.colspan}}'{{/if}} {{if isset($fields[$colData.field.name].type) && $fields[$colData.field.name].type == 'phone'}}class="phone"{{/if}}>
                    ...
                    {{if $inline_edit && !empty($colData.field.name) && ($fields[$colData.field.name].inline_edit == 1 || !isset($fields[$colData.field.name].inline_edit))}}<div class="inlineEditIcon col-1">
